...
we do not have.
A fifth fixture reaches no branch at all: a hostile IHDR, which the decode must refuse
before it sizes a buffer from it.

So this writes five minimal PNGs by hand, from zlib and struct alone.
Each is a few hundred bytes and targets exactly one behaviour.

The one output is png_fixtures.hh, which embeds each file's bytes as a C++ array.
No .png is written or committed: nexus has no test-data loader, so a file on disk
would need path resolution the repo has never had, and a fixture the test cannot
reach is not a fixture.
This script is what keeps the bytes auditable, and re-running it is the diff.
...
def hostile_ihdr() -> bytes:
    """65535x65535 RGBA16 in the IHDR, and a stub IDAT.

    Under 100 bytes on disk, and 32 GiB of pixels if the header is believed: spng_decoded_image_size
    answers from the IHDR alone, so the decode must weigh that size against its own ceiling before allocating.
    The per-axis cap does not catch it -- 65535 is exactly what spng_set_image_limits is given, and the test is `>`.
    """
    return png(ihdr(65535, 65535, 16, 6), idat(b"\x00" * 4))


FIXTURES = [
    ("png_indexed_trns", indexed_with_trns, "4x2 palette, tRNS on entry 0 -> RGBA8"),
    ("png_grey_1bit", grey_1bit, "8x2 one-bit grey -> G8, unpacked"),
    ("png_grey_trns", grey_8bit_with_trns, "4x2 8-bit grey, tRNS on 128 -> GA8"),
    ("png_rgb_adam7", rgb_adam7, "8x8 rgb, Adam7 across all seven passes"),
    ("png_hostile_ihdr", hostile_ihdr, "65535x65535 rgba16 header -> refused before allocation"),
]
...
def main() -> int:
    blocks = []
    for symbol, build, what in FIXTURES:
        data = build()
        print(f"{symbol}: {len(data)} bytes")
        blocks.append(as_cpp_array(symbol, data, f"{what} ({len(data)} bytes)"))
