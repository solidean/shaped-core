...
ASYNC_TEST("bcache acquire computes once and shares the result", main_thread)
{
    ...
    CHECK(blob_text((co_await a)) == "computed once");
    CHECK(blob_text((co_await b)) == "computed once");
...
ASYNC_TEST("bcache acquire serves a second caller from storage once the first has finished", main_thread)
{
    ...
    CHECK(blob_text((co_await a)) == "from compute");
    ...
    CHECK(blob_text((co_await b)) == "from compute");
...
ASYNC_TEST("bcache acquire runs one compute per distinct key", main_thread)
{
    ...
    CHECK(blob_text((co_await a)) == "first");
    CHECK(blob_text((co_await b)) == "second");
...
ASYNC_TEST("bcache acquire takes a plain blocking callback too", main_thread)
{
    ...
    CHECK(blob_text((co_await a)) == "synchronously produced");
    CHECK(calls == 1);

    (void)co_await f.idle();
    auto const b = f.cache().acquire(key, [&] { return make_blob("never runs"); });
    CHECK(blob_text((co_await b)) == "synchronously produced");
    CHECK(calls == 1);
}
...
ASYNC_TEST("bcache acquire releases a slot without disturbing a successor under the same key", main_thread)
{
    ...
    CHECK(blob_text((co_await b)) == "round two");
...
ASYNC_TEST("bcache acquire forgets an operation nobody is waiting on any more", main_thread)
{
    ...
    CHECK(blob_text((co_await b)) == "done"); // served from storage, not from a retained value
