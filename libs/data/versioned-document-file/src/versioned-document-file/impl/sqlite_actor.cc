...
class sqlite_connection_actor final : public cc::threaded_actor_impl<open_request,
                                                                     publish_request,
...
protected:
    ...
    void on_message(recovery_request msg) override
    {
        if (!_is_open)
        {
            msg.promise->push_error(as_async_error(cc::any_error(cc::string("recovering history in a store whose "
                                                                            "connection is closed"))));
            return;
        }

        auto writer = sqlite_writer(_db);
        auto applied = apply_recovery(writer, msg.job);
        if (applied.has_error())
            msg.promise->push_error(as_async_error(cc::move(applied).error()));
        else
            msg.promise->push_value(applied.value());
