...
- A **part** names one blob plus its format, and may carry a name.
...
- **Reclamation is one algorithm**: mark from the asset index, sweep the rest, with no reference counts to get wrong.

**Part names are for humans, and never load-bearing.**
Order is the contract.
A name exists so a dump is readable; nothing may key behaviour on it, because the moment something does, renaming a part becomes a format change.
