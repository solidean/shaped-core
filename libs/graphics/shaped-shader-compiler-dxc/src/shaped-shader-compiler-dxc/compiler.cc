...
#ifndef CC_OS_WINDOWS
/// Takes one reference to the UTF-8 locale and never gives it back.
///
/// DXC's ScopedLocale (its WinAdapter.h) builds and destroys a `locale_t` around EVERY string conversion it makes,
...
{
    // ScopedLocale's own candidates in its own order, so the reference held is the one it goes on to find.
    [[maybe_unused]] static locale_t const pinned = []
    {
        for (auto const* name : {"C.UTF-8", "C.utf8", "en_US.UTF-8"})
            if (auto const loc = newlocale(LC_CTYPE_MASK, name, locale_t(nullptr)); loc != locale_t(nullptr))
                return loc;
