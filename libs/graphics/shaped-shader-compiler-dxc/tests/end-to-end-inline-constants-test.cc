...
INVOCABLE_TEST("ssc::dxc + dx12 - inline constants drive Out[i] = i*scale + bias", (sg::context_handle const& handle))
{
    ...
    auto const g1_view
        = sg::named_view{.name = "Out", .views = {sg::buffer<u32>::from_raw(out1_buf).as_readwrite_buffer()}};
    auto const g2_view
        = sg::named_view{.name = "Out", .views = {sg::buffer<u32>::from_raw(out2_buf).as_readwrite_buffer()}};
