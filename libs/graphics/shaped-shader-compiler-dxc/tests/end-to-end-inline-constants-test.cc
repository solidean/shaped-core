...
INVOCABLE_TEST("ssc::dxc + dx12 - inline constants drive Out[i] = i*scale + bias", (sg::context_handle const& handle))
{
    ...
    auto const g1_view = sg::named_view{.name = "Out", .view = sg::buffer<u32>::from_raw(out1_buf).as_readwrite_buffer()};
    auto const g2_view = sg::named_view{.name = "Out", .view = sg::buffer<u32>::from_raw(out2_buf).as_readwrite_buffer()};
    ...
    // One list, two dispatches.
    // First: full replace {scale=3, bias=7} -> out1[i] = i*3 + 7.
    // Second: partial update of bias only (offset 4) to 100, with scale=3 carried over -> out2[i] = i*3 + 100.
    ...
    disp->compute.set_inline_constants(u32(100), isize(sizeof(u32)));
    disp->compute.dispatch_threads(count);
