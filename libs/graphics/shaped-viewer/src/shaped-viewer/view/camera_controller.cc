...
{
    if (auto const r = e.try_as_mouse_button(); r.has_value())
    {
    ...
    if (auto const r = e.try_as_mouse_move(); r.has_value())
    {
    ...
    if (auto const r = e.try_as_mouse_wheel(); r.has_value())
    {
        auto const& w = *r.value();
        if (w.delta[1] == 0.0f)
            return false;
    ...
    return false; // keys and text belong to the fps controller, or to the caller
}
...
bool fps_camera_controller::handle(sr::input_event const& e)
{
    ...
    return false; // text belongs to the caller
}
