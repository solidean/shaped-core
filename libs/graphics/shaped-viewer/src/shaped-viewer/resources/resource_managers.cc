...
mesh_id mesh_manager::acquire_deferred(cc::hash128 key, cc::optional<tg::aabb3f> bounds)
{
    if (auto const id = find_by_hash(key); id.has_value())
        return id.value();

    // Charged nothing, because nothing has been allocated: `supply` adds what the payload costs when it brings one.
    auto const id = insert(key, {.state = residency::pending, .bounds = bounds}, 0);
    _awaiting[id] = key;
    return id;
}

bool mesh_manager::is_awaiting_payload(mesh_id id)
{
    return _awaiting.contains(id);
}

void mesh_manager::_supply(mesh_id id,
                           cc::hash128 key,
                           cc::pinned_data<tg::pos3f const> const& positions,
                           cc::pinned_data<u32 const> const& indices)
{
    auto const* const expected = _awaiting.get_ptr(id);
    if (expected == nullptr)
        return; // already supplied, or never deferred — re-supplying is a comparison, not an error

    CC_ASSERT(*expected == key, "a deferred record is supplied under the key it was minted with");

    auto* const record = mutable_record(id);
    if (record == nullptr)
    {
        (void)_awaiting.erase(id);
        return; // evicted while awaiting; nothing is waiting on it any more
    }

    auto const indexed = !indices.empty();
    record->vertices = _ctx.persistent.create_buffer<tg::pos3f>(positions.size(), geometry_usage);
    record->is_indexed = indexed;
    record->triangle_count = (indexed ? indices.size() : positions.size()) / 3;

    auto transfers = pending_mesh{};
    transfers.vertices = _ctx.stream.data_to_buffer(record->vertices, positions);
    transfers.vertices.set_priority(impl::geometry_stream_priority);

    if (indexed)
    {
        record->indices = _ctx.persistent.create_buffer<u32>(indices.size(), geometry_usage);
        transfers.indices = _ctx.stream.data_to_buffer(record->indices, indices);
        transfers.indices.set_priority(impl::geometry_stream_priority);
    }
    else
    {
        auto up = _ctx.create_command_list();
        record->indices = _acquire_index_stand_in(*up);
        _ctx.submit_command_list(cc::move(up));
    }

    transfers.bytes = record->vertices.size_in_bytes() + (indexed ? record->indices.size_in_bytes() : 0);
    add_bytes(id, transfers.bytes);

    _settling[id] = cc::move(transfers);
    (void)_awaiting.erase(id);
}

void mesh_manager::supply(mesh_id id, triangle_data const& mesh)
{
    _supply(id, mesh.hash, mesh.positions, {});
}

void mesh_manager::supply(mesh_id id, indexed_triangle_data const& mesh)
{
    _supply(id, mesh.hash, mesh.positions, mesh.indices);
}
