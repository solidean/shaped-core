...
/// Everything a shader might want *on top of* that surface travels in the four open-ended lists rather than as fixed fields:
/// `attributes` per element of the geometry, `parameters` per mesh, `textures` per slot, and `flags` for the on/off decisions.
///
/// `material` is a thin id: material definitions live outside the mesh and are shared across many.
/// The material is what gives the lists their meaning — it decides which attribute names it samples, which parameters it reads, which texture slots it binds, and how the flags change what it emits.
...
struct sv::mesh
{
    ...
    /// arbitrary extra per-element data, looked up by name by the material
    ...
    /// per-mesh values the material reads by name
    cc::vector<mesh_parameter> parameters;
