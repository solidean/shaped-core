...
persistent per-view state                [in progress]  view_id keys what a view keeps: camera, controller, zoom, display name, last rect (impl::view_state) and its textures (view_renderer)
...
temporal accumulation                    [in progress]  a traced layer blends into a persistent rgba16f target and restarts when the hash of what it uploads changes
...
That state is split across **two caches, deliberately**:

- `viewer::impl::view_states` holds what a caller or the built-in controller reads and writes — the camera, the orbit, the zoom, the last rect, the hover / drag flags.
- `view_renderer::_persistent` holds the textures — one accumulation slot per traced layer, plus the view's composited target.

Both are `sv::impl::keyed_cache` instantiations ticked on `ctx.current_epoch()`, so one reclamation policy governs both and a view cannot survive in one while vanishing from the other.
The split earns its keep because `sg::render_routine::evict` drops the GPU half wholesale: an image restarts, and the camera the user framed it with survives.
It is also what lets `build_render_plan` stay a pure function — what the renderer already holds reaches it as a `view_history` value rather than a reach-back.

Neither half signals the other.
The trace decides for itself whether its image is still valid, by hashing the bytes it uploads, so losing the CPU state restarts the image rather than corrupting it.
Two tiers of idleness fall out of the same reason: an accumulation target is megabytes and is released after ~60 idle frames, while the identity behind it costs a few dozen bytes and survives ~240.
...
- **`render_routine` has no per-frame hook**, and no way to reach an instance under its lock without a `command_list`.
  That is why `view_renderer::begin_frame` takes a `cmd` it does not otherwise need; a second routine growing per-frame reclamation would justify an init-free `acquire_exclusive(context&)` in sg.
