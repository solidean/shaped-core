...
shaped-viewer now has a **first vertical slice**: a per-frame `viewer_definition` of views, each
ray-traced (flat-shaded PBR) into a target texture and blitted into a window. Raytracing-first —
raster is a later fallback / special case.
...
materials / lighting                     [planned]      one flat PBR material; the view holds typed light lists (area_lights) + an SH background; textures, more light kinds next
...
Ray tracing runs on **dx12 + DXR** (Windows), hardware or WARP. Vulkan RT is stubbed upstream in
shaped-graphics, so rendering is Windows-only for now — but the whole sv API compiles everywhere;
without a backend a routine simply acquires no shader and draws nothing.

## First library-extension seams (per the "living libraries" rule)

- **PBR/BRDF shading** is authored fresh in `shaders/pbr.hlsli`; a shared shader BRDF library is the
  natural home once a second consumer appears.
- **The id-pool now exists** as `sv::impl::lru_pool<Id, Record>` (budget + idle eviction, LRU); if a second
  library wants it, promoting a generational version into clean-core is the next step.
- **TLAS is rebuilt every frame** (refit/update not implemented in sg yet); `tlas_id` exists for a
  future prebuilt/persistent TLAS.
- **Texture upload/download/copy** are still future in sg, so the trace stays on the proven
  UAV-write-then-blit path and pixel-level tests await readback.
- **One mesh per view** — the trace binds the first item's vertex/material buffers; multi-mesh wants
  per-instance material/vertex indexing.
