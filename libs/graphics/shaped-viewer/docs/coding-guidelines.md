...
A test that traced ends with `sv_test::drain_ambient_work`, handed the fallback's compile, because a trace starts that compile on a pool worker where no queue shows it.
