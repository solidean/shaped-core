...
A test that traced is an `ASYNC_INVOCABLE_TEST` ending in `co_await cc::async_settled(sv::background_work(ctx))`: work the frames detached, which no queue shows.
