...
TEST("sv::serves splits the geometric frequencies by geometry kind")
{
    ...
    CHECK(!sv::serves(sv::geometry_kind::triangles, f::per_quadric));
    CHECK(!sv::serves(sv::geometry_kind::triangles, f::per_quadric_end));

    CHECK(sv::serves(sv::geometry_kind::quadrics, f::per_quadric));
    CHECK(sv::serves(sv::geometry_kind::quadrics, f::per_quadric_end));
...
TEST("sv::resolve_material takes a per_quadric attribute on a quadric batch")
{
    ...
    auto const attribute = sv::mesh_attribute::create("colour", sv::attribute_frequency::per_quadric, colours);
    ...
    CHECK(r.attributes[0].attribute->frequency == sv::attribute_frequency::per_quadric);
}
...
TEST("sv::resolve_material falls back when the geometry cannot number the frequency")
{
    ...
    auto const per_quadric = cc::array<tg::vec3f>::create_filled(2, tg::vec3f(0, 1, 0));
    auto const quadric_colour = sv::mesh_attribute::create("colour", sv::attribute_frequency::per_quadric, per_quadric);
...
TEST("sv::resolve_material leaves the texture ranks unreachable on a quadric")
{
    ...
    auto const uv = sv::mesh_attribute::create("uv", sv::attribute_frequency::per_quadric, uvs);
...
TEST("sv::generate_material_shader emits a flat load for per_quadric")
{
    auto const type = colour_type();
    auto const material = sv::material::create("m", sv::material_type_id::invalid, {});
    ...
    // `per_quadric` means exactly what `per_triangle` does — one element at PrimitiveIndex() — so it loads rather than blends.
...
TEST("sv::generate_material_shader blends the ends for per_quadric_end")
{
    auto const type = colour_type();
    auto const material = sv::material::create("m", sv::material_type_id::invalid, {});
    ...
    REQUIRE(r.attributes[0].attribute != nullptr);
    CHECK(r.attributes[0].attribute->frequency == sv::attribute_frequency::per_quadric_end);
...
TEST("sv::generate_material_shader forks a permutation on the geometry's frequency")
{
    ...
    auto const a = sv::resolve_material(
        type, material,
        quadric_set_with(sv::mesh_attribute::create("colour", sv::attribute_frequency::per_quadric, flat)));
    auto const b = sv::resolve_material(
        type, material,
        quadric_set_with(sv::mesh_attribute::create("colour", sv::attribute_frequency::per_quadric_end, ends)));
