...
// The capture tests configure the run through process environment variables, so they hold the env tag.
TEST("sv - a capture writes a complete image and ends the loop", nx::config::exclusive("env"))
{
    auto ctx_r = sg::create_dx12_context({.enable_debug_layer = true, .use_warp = true});
    if (ctx_r.has_error())
        SKIP("no Direct3D 12 device (hardware or WARP)");
...
TEST("sv - a capture nothing registered fails without writing", nx::config::exclusive("env"))
{
    auto ctx_r = sg::create_dx12_context({.enable_debug_layer = true, .use_warp = true});
    if (ctx_r.has_error())
        SKIP("no Direct3D 12 device (hardware or WARP)");
...
// WORKAROUND, not a thread requirement: the timed-out capture abandons async work the viewer never awaits.
// Off the main thread, in a full run, nexus then reports "left async work running" on this test; on main it has settled by the check.
// Awaiting or cancelling that work in sv is what removes main_thread here.
