...
TEST("sv - geometry can be a key before it is bytes, and the same record fills in")
{
    auto ctx_r = sg::create_dx12_context({.enable_debug_layer = true, .use_warp = true});
    if (ctx_r.has_error())
        SKIP("no Direct3D 12 device (hardware or WARP)");
    ...
    auto const positions = cc::vector<tg::pos3f>{tg::pos3f(0, 0, 0), tg::pos3f(1, 0, 0), tg::pos3f(0, 1, 0)};
    auto const payload = sv::triangle_geometry::create_from_positions(positions);
    auto const box = tg::aabb3f(tg::pos3f(0, 0, 0), tg::pos3f(1, 1, 0));

    // What a structure pass produces: a name, a placement, a box, and the key the payload will arrive under.
    auto const promise
        = sv::mesh_data{.name = "tri", .geometry = sv::triangle_geometry::create_deferred(payload.hash), .bounds = box};
    CHECK(promise.geometry.is_deferred());
    CHECK(!promise.geometry.is_empty()); // a promise is not nothing

    auto const deferred = m.create_mesh(promise);
    CHECK(m.meshes.get(deferred.geometry).state == sv::residency::pending);
    CHECK(m.meshes.is_awaiting_payload(deferred.geometry));

    // Sized by what the structure knew, which is what a placeholder is drawn at.
    CHECK(m.meshes.get(deferred.geometry).bounds.value().max == tg::pos3f(1, 1, 0));

    // Nothing is allocated for it: a record that is only a promise costs the budget nothing.
    CHECK(m.meshes.get(deferred.geometry).vertices.raw() == nullptr);

    // Waiting cannot finish it, because nothing was ever queued — a promise is not a transfer.
    m.wait_for_pending_uploads();
    CHECK(m.meshes.get(deferred.geometry).state == sv::residency::pending);

    // The payload arrives under the SAME key, so it fills that record rather than minting a second.
    auto const supplied = m.create_mesh({.name = "tri", .geometry = payload, .bounds = box});
    CHECK(supplied.geometry == deferred.geometry);
    CHECK(!m.meshes.is_awaiting_payload(deferred.geometry));

    m.wait_for_pending_uploads();
    CHECK(m.meshes.get(deferred.geometry).state == sv::residency::complete);
    CHECK(m.meshes.get(deferred.geometry).blas != nullptr);
    CHECK(m.meshes.get(deferred.geometry).triangle_count == 1);

    // Supplying again is a comparison rather than a second upload.
    auto const again = m.create_mesh({.name = "tri", .geometry = payload, .bounds = box});
    CHECK(again.geometry == deferred.geometry);
    CHECK(m.settling_count() == 0);
