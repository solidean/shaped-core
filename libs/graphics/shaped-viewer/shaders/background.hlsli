...
// elements to a float4 lane; index 0 is the constant (DC) term.
struct Background
{
    float4 sh[16];
};

// The coefficients are a PARAMETER rather than a global read here, because two pipelines share this file and a
// binding belongs to exactly one group.
// Each declares its own `background` buffer and passes `background.sh` in; DXC inlines these, so the array
// argument costs nothing over the global read it replaces.
...
float3 background_radiance(float4 sh[16], float3 d)
{
    float const x = d.x;
    float const y = d.y;
    float const z = d.z;
...
float3 background_irradiance(float4 sh[16], float3 n)
{
    ...
    float3 const L00 = sh[0].rgb;
    float3 const L1m1 = sh[1].rgb; // y
    float3 const L10 = sh[2].rgb;  // z
    float3 const L11 = sh[3].rgb;  // x
    float3 const L2m2 = sh[4].rgb; // x*y
    float3 const L2m1 = sh[5].rgb; // y*z
    float3 const L20 = sh[6].rgb;  // 3z^2 - 1
    float3 const L21 = sh[7].rgb;  // x*z
    float3 const L22 = sh[8].rgb;  // x^2 - y^2
