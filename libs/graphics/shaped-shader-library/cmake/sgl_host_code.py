...
def includes(entries: SglEntries) -> list[str]:
    ...
    if any(b["inline"] for _, b in entries.bindings):
        ...
        types = {m["type"] for _, b in entries.bindings if b["inline"] for m in b["members"]}
...
def emit_group(namespace: str, file: SglFile, binding: dict) -> str:
    name = binding["name"]
    out = [f"\nnamespace {namespace}\n{{\n"]
    ...
    for member in binding["members"]:
        access = "read and written by the shader" if member["mut"] else "read by the shader"
        out.append(f"    sg::bound_view {member['name']}; ///< `{member['type']}` buffer, {access}\n")
    out.append("\n")
...
def emit_group_impl(namespace: str, file: SglFile, binding: dict) -> str:
    name = binding["name"]
    qualified = f"{namespace}::{name}"
    out = [f"\n// `binding {name}` of {file.path}: the table the shader's buffers were numbered from.\n"]
    ...
    out.append(f"    views.reserve({len(binding['members'])});\n")
    for member in binding["members"]:
...
def emit_inline_impl(package: str, namespace: str, file: SglFile, binding: dict) -> str:
    ...
    for member in binding["members"]:
        ...
        if size != member["size"]:
            raise HostCodeError(
                f"shader package '{package}': '{file.path}' `@inline binding {name}` member '{member['name']}' is "
...
def emit_header(package: str, namespace: str, entries: SglEntries) -> str:
    ...
    for file, binding in entries.bindings:
        if binding["inline"]:
            ...
            out.append(emit_group(namespace, file, binding))
    return "".join(out)
...
def emit_source(package: str, namespace: str, entries: SglEntries) -> str:
    ...
    for file, binding in entries.bindings:
        if binding["inline"]:
            ...
            out.append(emit_group_impl(namespace, file, binding))
    return "".join(out)
