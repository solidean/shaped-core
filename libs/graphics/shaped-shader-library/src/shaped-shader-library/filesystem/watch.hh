...
namespace slib
{
/// Called when something under a watched prefix MAY have changed.
/// Fires from an arbitrary thread — often an OS watcher's — so it must be cheap and non-blocking: enqueue and return.
///
/// A hint to rescan, never a description of what changed.
...
/// That is what lets a subscriber keep a subscription beside the state its sink touches and drop it on the way out.
/// A sink landing one instruction late is the classic use-after-free in every file watcher.
///
/// Move-only.
...
public:
    ...
    /// Destroying it completes the unsubscribe, so an implementation's destructor is where the guarantee above is paid for.
    struct impl_base
    {
...
private:
    /// std::unique_ptr, not cc::unique_ptr: ownership is polymorphic through impl_base.
    /// Null when inert.
