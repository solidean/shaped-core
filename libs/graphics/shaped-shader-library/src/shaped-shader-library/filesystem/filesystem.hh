...
public:
    ...
    ///
    /// nullopt means "I cannot notify — poll me instead", which is where everything starts and where a platform without a watch backend stays.
    /// Returning it is a first-class answer, not a failure.
    ///
    /// Relaxed on purpose: a notification is a HINT TO RESCAN, never a description of what changed.
    /// An implementation may coalesce, may fire spuriously, and may watch a whole directory when a single file was asked for.
    /// A sibling's change firing the sink is allowed, and filtering is optional.
    /// revision() remains the source of truth.
    /// That is what lets an inotify queue overflow, or an editor that saves via write-temp-then-rename, degrade to "fire the sink" rather than to a missed reload.
