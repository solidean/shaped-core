...
///
/// This, with its watch backends, is the **only** part of slib that touches the disk or the OS — see libs/graphics/shaped-shader-library/docs/coding-guidelines.md for what rests on that.
///
...
public:
    ...
    ///
    /// A file prefix becomes a watch on its whole directory, so a sibling's change fires the sink too.
    /// filesystem::watch allows that on purpose; filtering here would only duplicate what revision() already settles.
...
private:
    /// `path` resolved against the root, or nullopt if it escapes.
    /// Native separators.
    ...
    /// The OS watcher, built on the first watch() and shared by every watch on this filesystem.
    /// Mutable so the const watch() can build it.
