...
/// Virtual-path string helpers.
/// Pure string manipulation — nothing here touches a filesystem, real or virtual.
/// slib paths are always '/'-separated and root-relative.

namespace slib::impl
{
/// Normalizes a virtual path: a backslash becomes '/', '.' segments drop, '..' pops the preceding segment, and leading, trailing and repeated slashes collapse.
/// The result is root-relative — never a leading '/'.
///
/// nullopt when the path escapes its root (a '..' with nothing left to pop).
/// That is what confines a mount to its own subtree, so it is the traversal guard rather than a tidiness pass.
...
/// Segment-wise, so "foobar" is not under "foo".
