...
/// Call sites reach an asset through its generated package symbol:
///
...
public:
    ...
    /// Cleared by the next successful promotion.
    ...
    /// Every virtual path this shader was built from — its source plus the includes that were resolved, across all formats compiled so far.
...
private:
    ...
    /// Promotes a ready `pending` into `current` and bumps the generation.
    /// Caller holds the lock.
    void promote_pending(shader_library& library, state& s, format_entry& entry) const;

    /// Weak, not a plain back-reference: this asset is reachable through a generated global, and those are statics that can outlive any library.
    /// Weak turns "acquire through a stale global" into a reported error instead of a dangling read.
    /// It does not keep the library alive — see the threading note on shader_library.
