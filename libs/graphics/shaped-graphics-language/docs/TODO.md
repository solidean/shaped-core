...
* **Texture methods.** The `DEBUG_sample_level`, `DEBUG_load` and `DEBUG_store` builtins stand in for them, for 2D shapes only.
  `sample` with implicit derivatives needs a builtin that only a pixel stage may call, and `size` needs HLSL's `GetDimensions`, which writes through out parameters; neither has a stand-in.
