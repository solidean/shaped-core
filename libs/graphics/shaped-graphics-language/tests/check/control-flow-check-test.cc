...
TEST("sgl check - if, while, for and loop check clean, and a condition is a bool")
{
    ...
    CHECK(body_reports("if k => return 1.0\nreturn k\n") == "type-mismatch 1:[k] a condition is a bool, got float\n");
    CHECK(body_reports("while n:\n    return 1.0\nreturn k\n") == "type-mismatch 1:[n] a condition is a bool, got int\n");
    CHECK(body_reports("for i in 0 ..< k:\n    return 1.0\nreturn k\n")
          == "type-mismatch 1:[k] a for runs over int, got float\n");
    CHECK(body_reports("for i : float in 0 ..< n:\n    return 1.0\nreturn k\n")
          == "type-mismatch 1:[float] a for runs over int, got float\n");
}
...
TEST("sgl check - and, or and not are the language's own, and take bools")
{
    ...
    CHECK(body_reports("if k and k < 1.0 => return 1.0\nreturn k\n")
          == "no-matching-overload 1:[k and k < 1.0] operator and(float, bool)\n");
    CHECK(body_reports("if not n => return 1.0\nreturn k\n") == "no-matching-overload 1:[not n] operator not(int)\n");
}
...
TEST("sgl check - a comparison chain resolves each operator")
{
    ...
    CHECK(body_reports("if 0.0 <= k < n => return 1.0\nreturn k\n")
          == "no-matching-overload 1:[0.0 <= k < n] operator <(float, int)\n");
}
...
TEST("sgl check - an assignment names a mutable local, or a member of one")
{
    ...
    CHECK(body_reports("let v = k\nv = 1.0\nreturn v\n")
          == "not-assignable 1:[v] v is immutable; `let mut` declares a local an assignment may name\n");
    CHECK(body_reports("k = 1.0\nreturn k\n") == "not-assignable 1:[k] k is a parameter, which is a value\n");
    CHECK(body_reports("for i in 0 ..< n:\n    i = 2\nreturn k\n")
          == "not-assignable 1:[i] i is immutable; `let mut` declares a local an assignment may name\n");
    CHECK(body_reports("let mut x = k\nx = n\nreturn x\n") == "type-mismatch 1:[n] expected float, got int\n");
    CHECK(body_reports("let mut x = k\nx += n\nreturn x\n") == "no-matching-overload 1:[x += n] operator +(float, int)\n");
    CHECK(body_reports("let mut b = k < 1.0\nb -= b\nreturn k\n")
          == "no-matching-overload 1:[b -= b] operator -(bool, bool)\n");
    CHECK(reports_for("binding frame:\n    exposure: float\nfun f(k: float){frame} -> float:\n    frame.exposure = k\n"
                      "    return k\n")
          == "not-assignable 1:[frame.exposure] frame is a binding, which no shader writes\n");
}

TEST("sgl check - a block is a scope: a local ends with it, and shadowing waits for its rule")
{
    CHECK(body_reports("if k > 0.0:\n    let x = k\nreturn x\n").starts_with("unknown-name 1:[x] x\n"));
    // two blocks beside each other share nothing
    CHECK(body_reports("if k > 0.0:\n    let x = 1.0\n    return x\nelse:\n    let x = 2.0\n    return x\n") == "");
    CHECK(body_reports("let x = k\nif k > 0.0:\n    let x = 1.0\nreturn x\n")
          == "unsupported-yet 1:[x] a local that shadows a local of an enclosing block\n");
    CHECK(body_reports("for k in 0 ..< n:\n    return 1.0\nreturn 0.0\n")
          == "unsupported-yet 1:[k] a local that shadows a local of an enclosing block\n");
    CHECK(body_reports("let x = k\nlet x = k\nreturn x\n") == "duplicate-declaration 1:[x] x\n");
}

TEST("sgl check - every path of a function that returns a value ends in a return")
{
    CHECK(body_reports("let x = k\n")
          == "missing-return 1:[f] f returns float, and a path through its body ends without a return\n");
    CHECK(body_reports("if k > 0.0 => return 1.0\n").starts_with("missing-return 1:[f]"));
    CHECK(body_reports("if k > 0.0:\n    return 1.0\nelse if k < -1.0:\n    return 2.0\n")
              .starts_with("missing-return 1:[f]"));
    CHECK(body_reports("if k > 0.0:\n    return 1.0\nelse:\n    return 2.0\n") == "");
...
TEST("sgl check - a function without a return type returns nothing")
{
    ...
    CHECK(reports_for("fun note(x: float):\n    return x\n")
          == "type-mismatch 1:[x] note has no return type, so its return has no value\n");
    CHECK(reports_for("fun half(x: float) -> float:\n    return\n")
          == "type-mismatch 1:[return] half returns float, and this return has no value\n");
    CHECK(reports_for("fun note(x: float):\n    print x\nfun f(k: float) -> float:\n    let y = note k\n    return k\n")
          == "type-mismatch 1:[note k] a let takes a value, and this is nothing\n");
    CHECK(reports_for("fun note(x: float):\n    print x\nfun f(k: float) -> float:\n    note k\n    return k\n") == "");
    // an arrow body has a value, and nothing infers its type yet
    CHECK(reports_for("fun half(x: float) => x * 0.5\n")
          == "unsupported-yet 1:[half] an arrow body without a written return type\n");
}
...
TEST("sgl check - a loop that is a value is left with break value, and any other with a bare break")
{
    CHECK(body_reports("let mut x = k\nlet y = loop:\n    x *= 2.0\n    if x > 8.0 => break x\nreturn y\n") == "");
    CHECK(body_reports("let mut x = k\nlet y = loop:\n    x *= 2.0\n    if x > 8.0 => break\nreturn x\n")
          == "type-mismatch 1:[break] this loop is a value, so its break carries one\n");
    CHECK(body_reports("let mut x = k\nlet y = loop:\n    if x > 8.0 => break x\n    if x < 0.0 => break n\nreturn y\n")
          == "type-mismatch 1:[n] an earlier break of this loop carries float, got int\n");
    CHECK(body_reports("let mut x = k\nloop:\n    x *= 2.0\n    if x > 8.0 => break x\nreturn x\n")
          == "type-mismatch 1:[x] nothing reads the value of this loop, so its break carries none\n");
    CHECK(body_reports("let y = loop:\n    return k\nreturn y\n")
              .contains("unsupported-yet 1:[loop:] a loop without a break as a value\n"));
}

TEST("sgl check - recursion is an error that names the loop, once")
{
    CHECK(reports_for("fun f(k: float) -> float:\n    return f(k)\n") == "recursive-call 1:[f(k)] f -> f\n");
    CHECK(reports_for("fun a(k: float) -> float => b(k)\n"
          ...
          == "recursive-call 1:[a(k - 1.0)] a -> b -> c -> a\n");
}
...
TEST("sgl check - an entry point that reaches recursion has no flat tree")
{
    ...
    CHECK(reports_of(checked) == "recursive-call 1:[down(k - 1.0)] down -> down\n");
...
TEST("sgl check - bindings are an effect: a caller lists what its callees read")
{
    ...
    CHECK(reports_for(head + "fun f(k: float) -> float => exposed k\n")
          == "binding-not-listed 1:[exposed k] exposed needs frame, which is not in the binding list of f\n");
    ...
    CHECK(reports_for(head + "fun middle(k: float){frame} -> float => exposed k\nfun f(k: float) -> float => middle k\n")
          == "binding-not-listed 1:[middle k] middle needs frame, which is not in the binding list of f\n");
}
...
TEST("sgl check - a flat node keeps its origin and the chain of call sites it was inlined through")
{
    ...
    for (auto const& x : e.exprs)
    {
        ...
        if (call == nullptr || call->intrinsic != sgl::check::builtin::multiply)
            continue;
