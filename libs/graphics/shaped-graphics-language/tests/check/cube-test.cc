...
TEST("sgl check - the cube checks without a diagnostic, and its whole dump is pinned")
{
    ...
    CHECK(sgl::check::dump(checked.module)
          == "(struct float builtin opaque)\n"
             ...
             "(fun normalize builtin (v : vec3) -> vec3)\n"
             "(fun dot builtin (a : vec3) (b : vec3) -> float)\n"
             "(fun saturate builtin (x : float) -> float)\n"
             "(fun transform_position builtin operator:* (m : mat4) (p : pos3) -> hpos4)\n"
             "(fun transform_direction builtin operator:* (m : mat4) (v : vec3) -> vec3)\n"
             "(fun scale_color builtin operator:* (c : float3) (s : float) -> float3)\n"
             "(fun multiply builtin operator:* (a : float) (b : float) -> float)\n"
             "(fun add builtin operator:+ (a : float) (b : float) -> float)\n"
