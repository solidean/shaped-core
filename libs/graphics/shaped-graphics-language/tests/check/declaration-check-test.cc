...
TEST("sgl check - a @builtin is keyed by its name, and an unknown name is unknown-builtin")
{
    ...
    CHECK(reports_of(swapped) == "unknown-builtin 1:[dot] dot\nunknown-builtin 1:[mat4] mat4\n");
}

TEST("sgl check - an opaque struct needs @builtin")
{
    CHECK(reports_for("struct handle\n") == "opaque-struct-needs-builtin 1:[handle] handle\n");
...
TEST("sgl check - a name is declared once, unless every declaration of it is a function")
{
    ...
    CHECK(reports_for("struct vec3:\n    x: float\n") == "duplicate-declaration 1:[vec3] vec3\n");
    CHECK(reports_for("binding dot:\n    x: float\n") == "duplicate-declaration 1:[dot] dot\n");
...
TEST("sgl check - a symbol that needs itself is a dependency cycle that names the loop")
{
    ...
    CHECK(reports_of(two) == "dependency-cycle 1:[a] a -> b -> a\n");
...
TEST("sgl check - names in type positions")
{
    ...
    CHECK(reports_for("struct a:\n    x: normalize\n")
          == "wrong-kind-of-name 1:[normalize] normalize is a function, and a type stands here\n");
...
{
    CHECK(reports_for("enum mode:\n    a\n    b\n") == "unsupported-yet 1:[enum mode:] enum\n");
    CHECK(reports_for("const k = 1.0\n") == "unsupported-yet 1:[const k = 1.0] const\n");
    CHECK(reports_for("type color = float3\n") == "unsupported-yet 1:[type color = float3] type alias\n");
    CHECK(reports_for("use brdf\n") == "unsupported-yet 1:[use brdf] use\n");
    CHECK(reports_for("sampler s:\n    filter = .linear\n") == "unsupported-yet 1:[sampler s:] sampler\n");
    CHECK(reports_for("fun id[T](x: T) -> T => x\n").starts_with("unsupported-yet 1:[id] a generic function\n"));
    CHECK(reports_for("struct a:\n    x: float\n    len => x\n") == "unsupported-yet 1:[len => x] a property\n");
    CHECK(reports_for("struct a:\n    x: float = 1.0\n") == "unsupported-yet 1:[1.0] a default value\n");
    CHECK(reports_for("binding b = constants\n") == "unsupported-yet 1:[constants] a binding composition\n");
    CHECK(reports_for("binding b:\n    t: texture2d[rgba8]\n") == "unsupported-yet 1:[texture2d[rgba8]] type arguments\n");
    CHECK(reports_for("@format(rgba8) struct a:\n    x: float\n")
          == "unsupported-yet 1:[format] the attribute @format on a struct\n");
...
TEST("sgl check - @operator hides the function's name, and takes one quoted operator")
{
    ...
    CHECK(reports_for(source) == "unknown-name 1:[multiply] multiply\n");

    CHECK(reports_for("@builtin @operator(1.0) fun multiply(a: float3, b: float3) -> float3\n")
          == "invalid-attribute-arguments 1:[operator] @operator takes one quoted operator, as in @operator(\"*\")\n");
    CHECK(reports_for("@builtin(1.0) struct mat4x\n")
              .contains("invalid-attribute-arguments 1:[(1.0)] @builtin takes no arguments\n"));
}
...
TEST("sgl check - the binding list is checked: every entry is a bare name of a binding")
{
    ...
    CHECK(reports_for("fun f(x: float){vec3} -> float => x\n") == "wrong-kind-of-name 1:[vec3] vec3 is no binding\n");
    CHECK(reports_for("binding b:\n    k: float\nfun f(x: float){b = b} -> float => x\n")
          == "unsupported-yet 1:[b = b] a binding entry that is not a bare name\n");
}
...
TEST("sgl check - an entry point's signature follows the rules of its stage")
{
    ...
    CHECK(entry_reports("@vertex fun vs(v: plain) -> vout:\n    return { p = hpos4(..v.p, 1.0) }\n")
          == "invalid-entry-point 1:[vs] the parameter of a @vertex fun is a @vertex struct\n");
    CHECK(entry_reports("@vertex fun vs(v: vin, w: vin) -> vout:\n    return { p = hpos4(..v.p, 1.0) }\n")
          == "invalid-entry-point 1:[vs] an entry point takes one struct parameter\n");
    CHECK(entry_reports("@vertex fun vs(v: vin) -> plain:\n    return { p = v.p }\n")
          == "invalid-entry-point 1:[vs] a @vertex fun returns a struct with exactly one @position field\n");
    ...
    CHECK(entry_reports("@vertex fun vs(v: vin) -> wrong:\n    return { a = float4(..v.p, 1.0) }\n")
          == "invalid-entry-point 1:[vs] the @position field of a @vertex fun is an hpos4\n");

    CHECK(entry_reports("@pixel fun ps(v: vout) -> target:\n    return { c = float4(..v.p) }\n") == "");
    CHECK(entry_reports("@pixel fun ps(v: vout) -> plain:\n    return { p = pos3(1.0, 1.0, 1.0) }\n")
          == "invalid-entry-point 1:[ps] a @pixel fun returns a @pixel struct\n");
    CHECK(entry_reports("@pixel fun ps(v: float) -> target:\n    return { c = float4(v, v, v, v) }\n")
          == "invalid-entry-point 1:[ps] the parameter of an entry point is a struct with fields\n");
    CHECK(entry_reports("@vertex @pixel fun ps(v: vout) -> target:\n    return { c = float4(..v.p) }\n")
          == "invalid-entry-point 1:[ps] an entry point has one stage\n");
}
...
TEST("sgl check - an entry point with an error has diagnostics and no flat tree")
{
    ...
    CHECK(reports_of(bad) == "unknown-name 1:[nope] nope\n");
