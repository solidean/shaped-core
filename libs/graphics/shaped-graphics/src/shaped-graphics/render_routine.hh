...
/// acquire(cmd) finds (or lazily creates) this routine's per-context instance in cmd.context().routines, initializes it, and returns it.
/// The customary shape is a static execute() that opens with `auto& self = acquire(cmd);` and works through `self`:
///
///   class my_routine : public sg::render_routine<my_routine>
///   {
///   public:
...
/// That is why acquire hands out a mutable reference rather than a const one.
///
...
///
/// State written in init_declare and merely *read* later is no exception:
/// a reload on another thread re-runs init_declare while this thread is recording, so that state belongs behind the same mutex.
///
...
{
public:
    /// The per-context instance for Derived, fully initialized (declare + materialize) at the current reload generation — the reference a static execute() works through.
