...
class command_list_upload_scope
{
    ...
    // Anything else is a compile error at the argument rather than a static_assert inside.
    // `T` is deduced only from the buffer, so the data argument converts to `span<T const>` the usual way — from a vector, a C array, a braced list.
    // Drop to the raw_buffer_handle overloads below, via `.raw()`, only for byte-addressed work.
public:
    /// Uploads a span of `T` into `dst` at `offset_in_elements`.
    ...
    /// Uploads a single `T` into `dst` at `offset_in_elements` — the one-element case.
    /// See bytes_to_buffer for the contract.
    template <class T>
...
public:
    ...
    /// Buffer must have buffer_usage::copy_dst, and offset_in_bytes + data.size() must fit it.
    ...
    /// Uploads a trivially-copyable contiguous range.
    /// `offset_in_elements` counts the range's value type; see bytes_to_buffer for the contract.
