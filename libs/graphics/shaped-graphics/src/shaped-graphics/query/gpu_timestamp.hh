...
///
/// Lifecycle: recorded into a command list, its raw tick lands on the host once that list has been submitted and its readback has finished on the GPU.
/// So read the result only after submitting the recording list — typically after a ctx.wait_for_ticks / wait_for_seconds, or a successful is_ready() poll.
/// Reading before submit reports not-ready.
/// Values are only meaningful as differences: two timestamps around some work give that work's GPU duration.
///
...
public:
    ...
    /// Backend seam: binds a timestamp to the shared per-heap future its tick is downloaded into.
    /// Plus the element `index` within that future, and the `tick_to_seconds` factor (1 / timestamp frequency).
    /// `heap_future` is shared by every timestamp of the same heap, and filled in place at submit.
    /// This is the single public seam a backend uses to construct a timestamp.
    ...
    /// Non-blocking poll: whether the tick has been read back to the host.
    ...
    /// The tick converted to seconds since an unspecified epoch if ready (polls), else nullopt.
    /// Only differences are meaningful.
    [[nodiscard]] cc::optional<double> try_get_seconds() const;

private:
    /// Shared future for the whole heap — one download per heap, with every timestamp of that heap aliasing it and indexing its own slot.
    /// Null iff invalid.
    /// Default-invalid until the recording list submits, then assigned in place by the backend.
