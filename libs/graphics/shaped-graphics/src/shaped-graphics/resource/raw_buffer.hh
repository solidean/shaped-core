...
public:
    ...
    /// A raw *structured* read-only view (SRV, `shape == structured`) over `byte_range` bytes, with an explicit element `stride_in_bytes`.
    ...
    /// A raw *structured* read-write view (UAV, `shape == structured`) over `byte_range` bytes, with an explicit element `stride_in_bytes`.
    /// `element_count` is `byte_range.size / stride`.
    ...
    // Draw-input views — a vertex or index buffer bound at draw time.
    // Not shader-visible, so they never go through views.hh or a binding group; bind them via `cmd.raster.bind_vertex_buffers` / `bind_index_buffer`.
    ...
    // `as_buffer` asserts the byte size is a whole number of `T`; `try_as_buffer` returns nullopt on a trailing partial element.
    // The inverse of `buffer<T>::raw()`, reached straight off the handle, and equivalent to `buffer<T>::from_raw(handle)`.
    // Defined in buffer.hh, where the wrapper is complete.
    template <class T>
    [[nodiscard]] auto as_buffer() const; // -> buffer<T>
    template <class T>
    [[nodiscard]] auto try_as_buffer() const; // -> cc::optional<buffer<T>>

    /// Register a callback to run once this buffer's GPU storage is released *and* its owning epoch has retired.
    /// The feedback point for reclaiming externally-owned backing memory, such as placed resources on a custom allocator.
    ...
    /// Expire the buffer now, releasing its GPU storage — deferred until it is no longer in flight.
    /// Idempotent, and const because expiry is a lifetime operation, not a change to the buffer's shape.
    void expire() const
    {
...
protected:
    ...
    // Range validation comes in pairs: an `is_valid_*` predicate, which the `try_` factories test, and an `assert_*` counterpart.
    // The counterpart reports the same contract one invariant at a time, which is what the asserting factories want.
    // Same split as `texture_description::is_valid` / `assert_valid` — keep the two in step.

    /// Whether a byte range fits the buffer: non-negative, and within bounds.
    /// Non-asserting twin of assert_byte_range.
    ...
    /// `storage_buffer_offset_alignment` says why those are the portable floors.
    ...
    /// Bounds-checks a strided storage view range: the storage rules above, a positive stride, and both size and offset a whole number of elements.
    /// The offset must *also* be stride-aligned, because a structured SRV/UAV addresses by element index — D3D12 computes `FirstElement = offset / stride`.
    /// An arbitrary byte offset into a heterogeneous buffer needs a raw view instead.
...
public:
    ...
    /// A high-water mark rather than a bool: a buffer streamed every frame would otherwise warn about the first
