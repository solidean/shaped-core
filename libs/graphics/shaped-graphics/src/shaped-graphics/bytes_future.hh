...
public:
    ...
    /// Checks completion, possibly advancing internal state to make progress.
    /// Returns is_ready().
    [[nodiscard]] virtual bool poll_ready() { return is_ready(); }

    /// Blocks until ready.
    /// Returns false when blocking cannot make progress — e.g. the recording command list has not been submitted yet, so waiting would stall the thread that must submit.
    [[nodiscard]] virtual bool wait() = 0;

    /// Backend seam: marks the download complete and wakes any blocked waiters.
    /// Call it once the destination bytes are valid.
    void mark_ready()
    {
...
public:
    ...
    /// Non-blocking poll: whether the bytes are ready.
    /// A download is ready only once its readback actor copy has run, and neither this nor an epoch advance forces that — ctx.wait_for(future) does.
    [[nodiscard]] bool is_ready() const { return _waiter != nullptr && _waiter->poll_ready(); }

    /// The result bytes if ready (polls), else nullopt.
    ...
    // members
private:
    /// Blocks until ready, then returns the bytes.
    /// Returns nullopt if invalid, or if blocking cannot make progress — the recording list is not yet submitted, or the download was cancelled.
...
public:
    ...
    /// The typed result if ready (polls).
...
private:
    /// Blocks until ready, then returns the typed result.
