...
INVOCABLE_TEST("sg error handling - buffer view factories validate usage and bounds", (sg::context_handle const& ctx))
{
    ...
    REQUIRE(ro != nullptr);
    CHECK_ASSERTS(sg::buffer<u32>::from_raw(ro).as_readonly_buffer({.offset = 0, .size = 1024})); // 1024 * 4 > 256 bytes
...
INVOCABLE_TEST("sg error handling - inline upload validates its arguments", (sg::context_handle const& ctx))
{
    ...
    byte payload[16] = {};
    auto const bytes = cc::span<byte const>(payload, 16);
...
INVOCABLE_TEST("sg error handling - binding group wiring errors throw", (sg::context_handle const& ctx))
{
    ...
    sg::named_view const unknown_name = {.name = "Nope", .views = {sg::buffer<u32>::from_raw(buf).as_readwrite_buffer()}};
    ...
    sg::named_view const wrong_kind = {.name = "Data", .views = {sg::buffer<u32>::from_raw(buf).as_readonly_buffer()}};
