...
// Wires the sg API up to the alias mechanism.
// For every INVOCABLE_TEST taking a context_handle, define an alias of the same name that expands to one scoped run per available backend.
// So `dev.py test "sg - ..."` runs that one API test on dx12, vulkan, … — whichever backends this binary was built with.
