...
#include <shaped-graphics/buffer.hh> // sg::buffer::size_in_bytes / usage (buffer_handle operator-> target)
#include <shaped-graphics/context.hh>
#include <shaped-graphics/types.hh>

// Backend-agnostic tests for persistent buffer allocation through `ctx->persistent.create_buffer`. Each is an
...
INVOCABLE_TEST("sg - allocates buffers across usages", (sg::context_handle ctx))
{
    ...
    {
        auto buf = ctx->persistent.create_buffer(1024, u);
        REQUIRE(buf.has_value());
...
INVOCABLE_TEST("sg - allocates a large buffer", (sg::context_handle ctx))
{
    ...
    auto buf = ctx->persistent.create_buffer(size, sg::buffer_usage::readwrite_buffer);
    REQUIRE(buf.has_value());
