...
ASYNC_INVOCABLE_TEST("sg - cross list ordering under a queue wait", (sg::context_handle const& ctx))
{
    ...
    auto constexpr element_count = 1024;
    auto constexpr half = element_count / 2;
    auto constexpr iterations = 100;

    auto const usage = sg::buffer_usage::copy_src | sg::buffer_usage::copy_dst;
    auto buffer = ctx->persistent.create_raw_buffer(element_count * isize(sizeof(u32)), usage);

    // Touched by both lists purely so each submit has an in-flight async transfer to order against.
    // Its contents are never checked.
    auto aux = ctx->persistent.create_raw_buffer(element_count * isize(sizeof(u32)), usage);

    cc::random rng;
    auto data = cc::vector<u32>::create_uninitialized(element_count);

    // Nothing is waited on inside the loop: the lists have to stay ordered while the queue is deep, which is when a
    // missing dependency actually shows.
    ...
    for (auto iteration = 0; iteration < iterations; ++iteration)
    {
        ...
        auto aux_data = cc::vector<u32>::create_uninitialized(element_count);
        for (auto& d : aux_data)
            d = rng.next_u32();
        ...
        reader->copy.buffer_data_region<u32>(
            {.src = buffer, .dst = buffer, .count = half, .src_offset = half, .dst_offset = 0});
        reader->copy.buffer_data_region<u32>({.src = aux, .dst = aux, .count = 8, .src_offset = 0, .dst_offset = 32});
        downloads.push_back(reader->download.data_from_buffer<u32>(buffer, 0, half));
        ctx->submit_command_list(cc::move(reader));

        expected.push_back(
            cc::vector<u32>::create_copy_of(cc::span<u32>(data).subspan({.start = half, .end = element_count})));
    }
    ...
    for (auto iteration = 0; iteration < iterations; ++iteration)
    {
        ...
        REQUIRE(downloaded.size() == isize(half));
