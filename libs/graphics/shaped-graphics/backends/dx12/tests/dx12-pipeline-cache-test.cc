...
// Exercises the sg-level built-in cache (ctx.cached) end to end on WARP: layout acquire dedups to one
// handle, compute-pipeline acquire dedups to one async node, the async resolves (driven inline here —
// no pool installed), and the cached pipeline actually dispatches correctly.
...
TEST("sg pipeline_cache - ctx.cached dedups layout + async compute pipeline")
{
    ...
    // Layout caching: identical bindings return the same shared handle.
    auto layout1 = ctx.cached.acquire_binding_layout(shader.bindings);
    auto layout2 = ctx.cached.acquire_binding_layout(shader.bindings);
    REQUIRE(layout1 != nullptr);
    CHECK(layout1.get() == layout2.get());

    // Compute-pipeline caching: identical (shader, layout) return the same async node.
    sg::compute_pipeline_description const desc{.shader = shader, .layout = layout1};
    ...
    auto group = ctx.persistent.create_binding_group(layout1, cc::span<sg::named_view const>(&out, 1));
...
TEST("sg pipeline_cache - a different shader yields a different pipeline node")
{
    ...
    sg::compiled_shader const shader = make_double_shader();
    auto layout = ctx.cached.acquire_binding_layout(shader.bindings);
    REQUIRE(layout != nullptr);

    sg::compute_pipeline_description const desc{.shader = shader, .layout = layout};
    auto base = ctx.cached.acquire_compute_pipeline(desc);

    // Same layout but shader content differs (entry point) -> different key -> different node.
    sg::compiled_shader altered = make_double_shader();
    altered.entry_point = "other_main";
    sg::compute_pipeline_description const desc2{.shader = altered, .layout = layout};
