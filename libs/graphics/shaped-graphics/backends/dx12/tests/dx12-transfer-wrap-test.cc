...
constexpr isize ring_bytes = 256; // tiny ring so a small transfer can be made to wrap
constexpr isize seam_gap = 64;    // bytes left before the seam when we park the cursor
...
TEST("sg dx12 - inline upload splits across the ring seam")
{
    ...
    CHECK(after.next_pos - before.next_pos == u64(xfer_bytes));
...
TEST("sg dx12 - inline download splits across the ring seam")
{
    ...
    CHECK(after.next_pos - before.next_pos == u64(xfer_bytes)); // split, not tail-waste
