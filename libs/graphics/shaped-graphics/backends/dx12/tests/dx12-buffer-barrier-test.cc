...
TEST("sg dx12 - an index-buffer read syncs on INDEX_INPUT, never VERTEX_SHADING")
{
    ...
    auto const b = after_copy_to(sg::pipeline_stage_flag::vertex, sg::access_flag::index_read);
...
TEST("sg dx12 - a vertex-buffer read still syncs on VERTEX_SHADING")
{
    ...
    auto const b = after_copy_to(sg::pipeline_stage_flag::vertex, sg::access_flag::vertex_read);
...
TEST("sg dx12 - one buffer read as both index and vertex widens to SYNC_DRAW")
{
    ...
    auto const b
        = after_copy_to(sg::pipeline_stage_flag::vertex, sg::access_flag::index_read | sg::access_flag::vertex_read);
...
TEST("sg dx12 - a copy-only barrier is untouched")
{
    ...
    auto const b = after_copy_to(sg::pipeline_stage_flag::copy, sg::access_flag::copy_read);
