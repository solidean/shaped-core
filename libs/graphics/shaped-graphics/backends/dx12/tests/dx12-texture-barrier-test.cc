...
TEST("sg dx12 - texture access declares layout transitions")
{
    ...
    auto b0 = acc.declare(slot, whole_of(d), sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_write,
                          sg::texture_layout::transfer_dst);
    REQUIRE(b0.size() == 1);
    CHECK(b0[0].barrier.needed);
    CHECK(b0[0].barrier.src_layout == sg::texture_layout::general);
    CHECK(b0[0].barrier.dst_layout == sg::texture_layout::transfer_dst);
    CHECK(sg::has_all(b0[0].barrier.dst_access, sg::access_flags::transfer_write));
    ...
    auto b1 = acc.declare(slot, whole_of(d), sg::pipeline_stage_flags::compute, sg::access_flags::shader_read,
                          sg::texture_layout::shader_read);
    REQUIRE(b1.size() == 1);
    CHECK(b1[0].barrier.src_layout == sg::texture_layout::transfer_dst);
    CHECK(b1[0].barrier.dst_layout == sg::texture_layout::shader_read);
}
...
TEST("sg dx12 - texture access fragments per subresource range")
{
    ...
    auto a = acc.declare(slot, mip0, sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_write,
                         sg::texture_layout::transfer_dst);
    REQUIRE(a.size() == 1);
    ...
    auto c = acc.declare(slot, whole_of(d), sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_read,
                         sg::texture_layout::transfer_src);
    CHECK(c.size() == 2);
}
...
TEST("sg dx12 - a non-final submit reverts the texture to its entry layout")
{
    ...
    (void)acc.declare(s0, whole_of(d), sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_write,
                      sg::texture_layout::transfer_dst);
    ...
    CHECK(revert[0].barrier.src_layout == sg::texture_layout::transfer_dst);
...
TEST("sg dx12 - the final submit promotes without reverting")
{
    ...
    (void)acc.declare(slot, whole_of(d), sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_write,
                      sg::texture_layout::transfer_dst);
    ...
    auto again = acc.declare(slot2, whole_of(d), sg::pipeline_stage_flags::transfer, sg::access_flags::transfer_write,
                             sg::texture_layout::transfer_dst);
...
TEST("sg dx12 - d3d12_layout_from maps the layouts")
{
    ...
    CHECK(dx12::d3d12_layout_from(sg::texture_layout::shader_read) == D3D12_BARRIER_LAYOUT_SHADER_RESOURCE);
    CHECK(dx12::d3d12_layout_from(sg::texture_layout::storage) == D3D12_BARRIER_LAYOUT_UNORDERED_ACCESS);
    CHECK(dx12::d3d12_layout_from(sg::texture_layout::transfer_dst) == D3D12_BARRIER_LAYOUT_COPY_DEST);
    CHECK(dx12::d3d12_layout_from(sg::texture_layout::transfer_src) == D3D12_BARRIER_LAYOUT_COPY_SOURCE);
}
...
TEST("sg dx12 - emits well-formed texture barriers on WARP")
{
    ...
    // track_texture_access): general → transfer_dst, then transfer_dst → shader_read.
    auto emit = [&](cc::span<dx12::dx12_subresource_barrier const> barriers)
    {
        for (auto const& sb : barriers)
            dx12::emit_texture_barrier(cmd.value()->_list.Get(), dtex->_resource.Get(), sb.range, sb.barrier);
    };
    emit(dtex->declare_texture_access(cmd.value()->slot(), range, sg::pipeline_stage_flags::transfer,
                                      sg::access_flags::transfer_write, sg::texture_layout::transfer_dst));
    emit(dtex->declare_texture_access(cmd.value()->slot(), range, sg::pipeline_stage_flags::compute,
                                      sg::access_flags::shader_read, sg::texture_layout::shader_read));
