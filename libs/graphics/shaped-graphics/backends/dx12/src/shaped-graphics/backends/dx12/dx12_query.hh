...
/// Backend GPU-query system: a free-list pool of small (SlotsPerHeap-slot) ID3D12QueryHeaps.
/// A command list leases a heap on demand and bump-allocates slots as queries are recorded.
/// At submit it resolves the heap into a transient buffer and starts one inline readback — see dx12_command_list::finalize_queries_before_close.
...
public:
    ...
    /// A list recording more timestamps than this simply leases additional heaps.
    static constexpr int SlotsPerHeap = 4096;

    explicit dx12_query_system(dx12_context& ctx) : _ctx(ctx) {}

    /// Caches the direct queue's timestamp frequency — the tick→seconds factor — and marks timestamp support.
    /// Called once during context bring-up, after the queue exists.
    /// Returns a dx12 error when the frequency query fails.
    ...
    /// Leases a heap of `type` with all slots free.
    /// Pulls from the free list, or creates a new heap.
    [[nodiscard]] cc::unique_ptr<dx12_query_heap_lease> acquire_heap(dx12_query_heap_type type);

    /// Returns a heap to the pool: resets next_slot and installs a fresh, invalid shared_future for the next leaseholder.
    ...
    /// Drops all pooled heaps.
    /// Called from context shutdown.
    void shutdown();
