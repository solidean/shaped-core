...
/// A small non-shader-visible CPU descriptor heap for RTV / DSV descriptors.
/// Unlike dx12_descriptor_heap — a shader-visible CBV/SRV/UAV heap with a GPU handle and epoch-scoped ring — RTV/DSV descriptors are CPU-only.
...
struct sg::backend::dx12::dx12_cpu_descriptor_heap
{
    ...
    /// Reserves one descriptor slot, reusing a freed slot when available.
    /// Returns cpu_descriptor_slot::invalid when the heap is exhausted — a recoverable failure the caller reports as an error.
