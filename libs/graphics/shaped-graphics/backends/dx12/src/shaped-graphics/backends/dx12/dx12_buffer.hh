...
public:
    ...
    // Two per-resource cross-queue sync stamps that make the CPU timeline (submit → async upload → submit) mirror GPU ordering between the direct queue and the async-upload copy queue.
    // Both only ever grow and are never reset — a stale value just yields a cheap already-satisfied wait.
    // Both are mutable + atomic, so they can be stamped through the const buffer handle from any thread.
    // Distinct from the access-state tracking below: that orders direct-queue lists against each other, while these order a copy queue against the direct queue.

    // Forward: highest completion value an ASYNC upload (ctx.upload, not the inline cmd.upload) here will signal on its copy queue.
    // A later direct-queue list that touches this buffer waits for it at submit, so it sees the async write.
    mutable std::atomic<u64> _pending_async_upload_value = 0;

    // Reverse: highest direct-queue submission token of a command list that used this buffer.
    // An async upload here defers its copy until this token completes, so it never overwrites the buffer while an earlier-submitted list still uses it.
    // The async download (ctx.download) reuses it too: its readback defers behind this token, so it reads only after the last direct-queue writer has finished.
    mutable std::atomic<u64> _last_used_submission_token = 0;

    // Forward for the async DOWNLOAD (ctx.download): highest completion value a pending async readback here will signal on the download completion fence.
    // That is dx12_download_async_system::_completion_fence, signaled once the copy-queue read has finished.
    ...
    // Unlike a texture there is no canonical between-lists state and no promote/revert: a dx12 buffer's layout is always `general`, since D3D12 decays buffers to COMMON at ExecuteCommandLists.
    // Cross-list ordering rides on that decay, so only *intra-list* hazards ever produce a barrier — each list seeds a fresh state and clears its slot at submit/drop.
    struct access_slot
    {
    ...
    /// Accumulate one declared `stages`/`access` for `slot` into the next-op state, seeding a fresh state on first touch, without emitting anything.
    /// Call once per binding — a buffer bound several times to the same op declares several times, and `flush_access` then merges them into one barrier.
    /// Thread-safe.
    ...
    /// The command list uses it to enqueue the buffer for the pre-op barrier flush exactly once, however many times it is bound.
    /// `flush_access` clears it.
    /// Thread-safe.
    [[nodiscard]] bool mark_pending_barrier(sg::command_list_slot slot) const;

    /// Flush the accesses declared for `slot` since the last flush and return the single intra-list barrier that satisfies their union; `needed == false` when it is a freebie.
    /// Called once per op, before it, after all its bindings are declared.
    /// Also clears the pending-barrier flag.
    /// Thread-safe.
    ...
    /// The command list uses it to add the buffer to its touched set exactly once, in O(1), replacing a linear scan.
    /// Thread-safe.
    [[nodiscard]] bool mark_recorded(sg::command_list_slot slot) const;

    /// Finalize `slot` when its command list is submitted: just clear the slot, freeing the tracking slot for a future list.
    /// A buffer has no layout to promote or revert, no between-lists state to carry, and needs no barriers.
    /// Thread-safe.
    ...
    /// Identical to `finalize_slot` for a buffer, which has nothing to promote or revert.
    /// Thread-safe.
