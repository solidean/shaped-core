...
cc::result<cc::unit> dx12_download_inline_system::initialize(isize capacity)
{
    ...
    _ring.lock([](ring_state& s) { s.current_epoch_copies = std::make_shared<std::atomic<isize>>(0); });
                                                              ...
                                                              isize size)
{
    ...
    while (!download.is_finished())
    {
        isize const off = isize(cursor % u64(_capacity));
        isize const budget = _capacity - off; // contiguous bytes to the seam
...
void dx12_download_inline_system::wait_for_submission(sg::submission_token token)
{
    ...
    {
        HRESULT const hr = _ctx._submission_fence->SetEventOnCompletion(u64(token), _wait_event);
        CC_ASSERT(SUCCEEDED(hr), "ID3D12Fence::SetEventOnCompletion failed");
        WaitForSingleObject(_wait_event, INFINITE);
    }
}

void dx12_download_inline_system::on_copy_done(std::shared_ptr<std::atomic<isize>> const& epoch_copies)
{
...
void dx12_download_inline_system::set_budget(isize capacity)
{
    CC_ASSERT(capacity > 0, "download ring capacity must be positive");
...
{
    u64 const freed = _freed_pos.load(std::memory_order_acquire);
    return _ring.lock([&](ring_state& s) { return debug_cursor_snapshot{s.next_pos, freed, _capacity}; });
}

void dx12_download_inline_system::debug_set_cursor(u64 pos)
{
