    ...
    sg::raytracing_shader_table_description const& desc)
{
    ...
    for (auto h : desc.raygen)
        CC_ASSERT(u32(h) < u32(pipeline->shader_ids_raygen.size()), "raygen handle out of range");
    for (auto h : desc.miss)
        CC_ASSERT(u32(h) < u32(pipeline->shader_ids_miss.size()), "miss handle out of range");
    for (auto h : desc.hit)
        CC_ASSERT(u32(h) < u32(pipeline->shader_ids_hit.size()), "hit handle out of range");
    for (auto h : desc.callable)
        CC_ASSERT(u32(h) < u32(pipeline->shader_ids_callable.size()), "callable handle out of range");

    // A record holds only the 32-byte identifier; records align to 32, sections to 64.
    constexpr isize record_size = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
    isize const stride = align_up(record_size, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
    auto const section_size
        = [stride](isize count) { return align_up(count * stride, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT); };

    isize const raygen_section = section_size(desc.raygen.size());
    isize const miss_section = section_size(desc.miss.size());
    isize const hit_section = section_size(desc.hit.size());
    isize const callable_section = section_size(desc.callable.size());

    isize const raygen_start = 0;
    isize const miss_start = raygen_start + raygen_section;
    isize const hit_start = miss_start + miss_section;
    isize const callable_start = hit_start + hit_section;
    isize const total = callable_start + callable_section;

    // Assemble the CPU byte image: each record is the pipeline's stored identifier at the record's stride.
    auto image = cc::vector<byte>::create_filled(total, byte{0});
    auto const write_section
        = [&](isize start, auto const& handles, cc::vector<dx12_raytracing_pipeline::shader_identifier> const& ids)
    {
        isize offset = start;
        for (auto h : handles)
        {
