...
public:
    ...
    webgpu_binding_group_layout(cc::hash128 hash,
    ...
    [[nodiscard]] WGPUBindGroupLayout raw() const { return _layout.get(); }

    /// Per slot in bindings(): the sampler a static sampler binding binds, null otherwise.
    [[nodiscard]] cc::span<WGPUSampler const> slot_samplers() const { return _slot_samplers; }

private:
    wgpu_bind_group_layout _layout;
    cc::vector<WGPUSampler> _slot_samplers; // owned by the context's sampler cache
};
...
public:
    ...
    [[nodiscard]] WGPUPipelineLayout raw() const { return _layout.get(); }
    ...
    [[nodiscard]] bool has_reserved_group() const { return bool(_reserved_layout); }
    ...
    [[nodiscard]] WGPUBindGroup empty_group() const { return _empty_group.get(); }

    wgpu_pipeline_layout _layout;
    cc::vector<webgpu_binding_group_layout_handle> _groups;
    int _slot_count = 0;
    isize _inline_constants_bytes = 0;

    wgpu_bind_group_layout _reserved_layout;
    wgpu_bind_group_layout _empty_layout;
    wgpu_bind_group _empty_group;
    ...
    cc::vector<reserved_sampler> _reserved_samplers;
