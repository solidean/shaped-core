...
// sr's shader package, checked against the shaders it was generated from.
//
...
TEST("sr - the generated shader tables still describe the shaders they came from")
{
    auto const message = sr::shaders::self_check();
    CHECK(message.empty());
    if (!message.empty())
        FAIL(message);
}
