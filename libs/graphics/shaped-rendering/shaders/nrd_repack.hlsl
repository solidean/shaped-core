...
[numthreads(8, 8, 1)] void main_cs(uint3 id : SV_DispatchThreadID)
{
    ...
    gDiffuseRadianceHitDistance[id.xy]
        = REBLUR_FrontEnd_PackRadianceAndNormHitDist(gDiffuse.Load(p).rgb, diffuse_norm_hit_distance, true);
    gSpecularRadianceHitDistance[id.xy]
        = REBLUR_FrontEnd_PackRadianceAndNormHitDist(gSpecular.Load(p).rgb, specular_norm_hit_distance, true);
}
