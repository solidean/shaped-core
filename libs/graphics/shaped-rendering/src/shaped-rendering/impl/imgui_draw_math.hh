...
/// Deliberately free of imgui types: the caller converts ImVec2 / ImVec4 / ImTextureRect at the boundary,
/// which keeps these functions checkable against hand-computed numbers.
...
/// `display_pos` is nonzero only under multi-viewport; folding it in costs nothing and is what a
/// per-viewport pass would otherwise have to add.
[[nodiscard]] imgui_ortho_constants compute_ortho_constants(tg::pos2f display_pos, tg::vec2f display_size);

/// Converts one ImDrawCmd::ClipRect into a scissor rect in framebuffer pixels: translate out of display
/// space, scale to framebuffer pixels, then clamp to the target.
/// Returns nothing when the result is empty after clamping — a fully-offscreen draw, which must be
/// skipped rather than submitted as a zero-area scissor.
...
/// Copies the `size` rect at `offset` out of a `pitch`-strided image into a freshly allocated, pinned,
/// tightly packed buffer — the layout sg's texture uploads require.
///
/// A fresh allocation per call is deliberate, not wasteful: ctx.upload is fire-and-forget and holds the
/// pin until the copy has consumed it, so a buffer reused across uploads would be rewritten while a copy
