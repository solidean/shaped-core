...
///
/// A render routine (the "everything that draws is a routine" rule): it owns the blit raster pipeline (one per target format, via a keyed_pipeline_cache), its layout, and a linear-clamp sampler.
/// All are built in `init_declare`.
