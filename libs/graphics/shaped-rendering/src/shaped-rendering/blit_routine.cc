...
void blit_routine::init_declare(sg::context& ctx)
{
    ...
    auto const pipeline_layout = _state.lock(
        [&](state& s)
        {
            ...
            s.group_layout = ctx.cached.acquire_binding_group_layout(compiled_ps->bindings);

            s.sampler = sg::sampler{.min_filter = sg::sampler_filter::linear,
    ...
    self._state.lock(
        [&](state& s)
        {
            ...
            auto const group = ctx.transient.create_binding_group(
                s.group_layout, {{.name = "source_texture", .view = src.as_readonly_view()}},
                {{.name = "linear_sampler", .sampler = s.sampler}});
