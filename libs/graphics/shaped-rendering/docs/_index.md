...
Documentation hub for shaped-rendering.
...
shaped-rendering is built on [shaped-graphics](../../shaped-graphics/readme.md) and [shaped-shader-library](../../shaped-shader-library/readme.md);
it hosts the concrete render routines and the SDL3-backed window abstraction (`sr::window_system` / `sr::window`).
The render-routine *framework* lives in shaped-graphics (see [its render-routines doc](../../shaped-graphics/docs/render-routines.md)).

## Topics

- [denoising](denoising.md) — one call for every denoiser: the members, the reconstruction contract, and how a denoiser meets a progressive path tracer.
...
- Namespace `sr`; depends on shaped-graphics + shaped-shader-library (and transitively typed-geometry + clean-core).
- Code follows the repo [coding-guidelines](../../../../docs/coding-guidelines.md) plus the sr-specific [coding-guidelines](coding-guidelines.md).
