...
struct nx::fuzz::executed_operation
{
    ...
    /// Where each argument is read from.
    /// For a cc::random& argument the slot's type is the machine's random type and the value index is unused, since the generator is synthesized from `state`.
    cc::vector<typed_value_index> arg_slots;

    /// Where the result is written; type == invalid for a void operation.
    /// A value index equal to the return type's current slot count means "append a new slot".
