...
//
// The engine models a fuzz program in SSA-like form: every value lives in a "slot" addressed by (type, value) indices.
// Every step is an operation reading some slots and writing one.
// The three index roles are distinct strong enums so the compiler rejects mixing them up, and each carries its own `invalid` sentinel — "none", and a void return on type_index.
// Crossing into the underlying int for subscripting or arithmetic is an explicit `int(x)` at the use site.
