...
{
    for (int i = 0; i < _machine.num_operations(); ++i)
        if (_exec_count[i] < _machine.op(op_index(i)).op->execute_at_least_times())
            return true;
    return false;
}
...
bool fuzz_runner::eligible(op_index op, fuzz_machine::state const& s) const
{
    ...
    if (_exec_count[int(op)] >= oi.op->execute_at_most_times())
        return false;
...
bool fuzz_runner::try_roll(op_index op, fuzz_machine::state const& s, executed_operation& out) const
{
    ...
    out.operation = op;
    // The state is rolled for every operation (even ones that ignore it) so the RNG stream order is
    // stable; operations taking a cc::random& are rebuilt from it, which is what makes runs replayable.
    out.state = _rng.next_u64();

    for (type_index at : oi.arg_types)
    {
        if (_machine.is_random_type(at))
        {
            out.arg_slots.push_back(typed_value_index{at, value_index(0)});
            continue;
        }
        int const count = s.count_of(at);
        if (count == 0)
            return false;
        out.arg_slots.push_back(typed_value_index{at, value_index(_rng.uniform(0, count - 1))});
    }

    if (!_machine.preconditions_fulfilled(s, out))
        return false;

    if (oi.return_type != type_index::invalid)
    {
        int const count = s.count_of(oi.return_type);
        // [0, count] inclusive: drawing `count` appends a new slot, which is how the state grows.
        out.return_slot = typed_value_index{oi.return_type, value_index(_rng.uniform(0, count))};
    }
    else
    {
        out.return_slot = typed_value_index{type_index::invalid, value_index::invalid};
    }
    return true;
}
...
bool fuzz_runner::create_next_execution(fuzz_machine::state const& s, executed_operation& out)
{
    ...
    for (int i = 0; i < _machine.num_operations(); ++i)
        if (_exec_count[i] < _machine.op(op_index(i)).op->execute_at_least_times() && eligible(op_index(i), s))
            open_eligible.push_back(op_index(i));
    ...
    for (int i = 0; i < _machine.num_operations(); ++i)
        if (!_machine.op(op_index(i)).is_invariant && eligible(op_index(i), s))
            all_eligible.push_back(op_index(i));
