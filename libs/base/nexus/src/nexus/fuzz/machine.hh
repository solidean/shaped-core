...
#include <string> // std::string: carried out of the CC_ASSERT handler
#include <typeindex>
...
struct fuzz_machine
{
    struct op_info
    {
        ...
        type_index return_type = invalid_index; // invalid for void
    ...
    struct state
    {
        ...
        [[nodiscard]] int count_of(type_index t) const { return int(values_by_type[t].size()); }
    };
    ...
    [[nodiscard]] op_info const& op(op_index i) const { return _operations[i]; }
    [[nodiscard]] type_info const& type(type_index i) const { return _types[i]; }

    [[nodiscard]] type_index random_type() const { return _random_type; }
    [[nodiscard]] bool is_random_type(type_index t) const { return t == _random_type && _random_type != invalid_index; }

    /// Looks up the interned index of a runtime type, or invalid_index if the machine never saw it.
...
private:
    ...
    type_index _random_type = invalid_index;
};
} // namespace nx::fuzz
