...
|---|---|---|---|
| `std::unordered_map` | [execute.cc](../src/nexus/tests/execute.cc) — the executor's section tree | `cc::map` | `cc::map` has landed; the section tree has not migrated to it yet. |
...
- **`cc::string` out of the assertion handler.** `cc::impl::assertion_info` now exposes
  ...
  ([tests/check.hh](../src/nexus/tests/check.hh)) — no `<string>`/`<functional>` left in the
  assertion path.
