...
//
// Holds the output sink, the value→text seam (the only boundary touching std::to_chars, in format.cc),
// the decoration helpers, the built-in cc::formatter<T> specializations, and the (no-ADL) per-argument
// dispatch + render plumbing. The public surface is cc::formatter<T> (formatter.hh) and the cc::format /
