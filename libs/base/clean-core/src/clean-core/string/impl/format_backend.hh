...
/// Shared integer rendering: picks base/prefix from the spec and emits via format_write_decorated_number.
...
template <class>
inline constexpr bool format_always_false = false;
} // namespace cc::impl
...
void format_value(format_sink const& out, string_view spec, T const& v)
{
    ...
    format_spec const s = format_parse_spec(spec);

    if constexpr (std::is_same_v<U, bool>)
    {
        if (s.presentation == '\0' || s.presentation == 's')
            format_write_decorated_text(out, s, v ? cc::string_view("true") : cc::string_view("false"),
                                        format_align_t::left);
        else
    ...
    else if constexpr (std::is_same_v<U, char>)
    {
        if (s.presentation == '\0' || s.presentation == 'c')
            format_write_decorated_text(out, s, cc::string_view(&v, 1), format_align_t::left);
        else
        {
            int const iv = int(v);
            bool const neg = iv < 0;
    ...
    else if constexpr (std::is_same_v<U, byte>)
    {
        ...
        if (s.presentation == '\0') // default byte rendering matches cc::to_string(byte): "0x" + two uppercase hex digits
        {
            ...
            format_write_decorated_number(out, s, false, cc::string_view("0x"), cc::string_view(digits, 2));
        }
        else
    ...
    else if constexpr (formattable_ptr<U>)
    {
        ...
        isize const n = format_chars_from_ptr(cc::span<char>(buf, format_chars_int_max), static_cast<void const*>(v));
        format_write_decorated_text(out, s, cc::string_view(buf, n), format_align_t::left);
    }
    else if constexpr (std::is_convertible_v<U const&, cc::string_view>)
    {
        format_write_decorated_text(out, s, cc::string_view(v), format_align_t::left);
    }
    else
    {
        static_assert(cc::impl::format_always_false<T>, "cc::format_value: type is not a standard-formattable "
                                                        "built-in");
    }
}
...
void format_one(format_sink const& sink, string_view spec, T const& v)
{
    ...
    {
        static_assert(format_always_false<T>,
                      "type is not formattable: specialize cc::custom::formatter<T> or add a member "
                      "to_string()");
    }
}
...
void format_dispatch(format_sink const& sink, string_view fmt, Args&&... args)
{
    ...
    format_render(sink, fmt, span<format_arg_entry const>(entries, isize(sizeof...(Args))));
}
} // namespace cc::impl
