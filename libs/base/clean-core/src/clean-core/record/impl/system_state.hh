...
/// Runs `f` with the consumer excluded, so no chunk can be recycled underneath a reader.
///
/// **Waits**, which is what separates it from the crash path.
/// The actor holds this lock for a whole drain pass, so a caller may sit here for one — fine for a hang report,
/// where the process is healthy in every respect except that one thread is not moving, and wrong for a fault
/// handler, where the thread holding it may be the one that died.
///
...
void with_consumer_paused(cc::function_ref<void()> f);
