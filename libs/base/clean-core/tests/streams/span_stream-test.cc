...
{
    byte const data[5] = {b(10), b(11), b(12), b(13), b(14)};
    ...
    CHECK(bytes_equal(s.ready_bytes(), cc::span<byte const>(data)));
    ...
    CHECK(bytes_equal(s.ready_bytes(), cc::span<byte const>({b(12), b(13), b(14)})));
...
{
    byte const data[4] = {b(1), b(2), b(3), b(4)};
    auto adapter = cc::span_read_stream_adapter(cc::span<byte const>(data));
    cc::seekable_read_stream s = adapter.stream();
...
{
    byte buf[4] = {b(0), b(0), b(0), b(0)};
    cc::span_write_stream_adapter adapter(buf);
    cc::seekable_write_stream s = adapter.stream();

    REQUIRE(s.write(cc::span<byte const>({b(1), b(2), b(3), b(4)})).has_value());
    // bytes land directly in the destination span (unbuffered)
    byte const expected[4] = {b(1), b(2), b(3), b(4)};
    CHECK(bytes_equal(cc::span<byte const>(buf), cc::span<byte const>(expected)));
}
...
TEST("span_stream - bounded write errors when the span is full")
{
    ...
    auto r = s.write(cc::span<byte const>({b(1), b(2), b(3), b(4), b(5)}));
    CHECK(r.has_error());
}

TEST("span_stream - read_write shares a single cursor")
{
    byte buf[4] = {b(1), b(2), b(3), b(4)};
    ...
    cc::vector<byte> head = cc::vector<byte>::create_defaulted(2);
    REQUIRE(s.read(head).value() == 2);
    CHECK(bytes_equal(head, cc::span<byte const>({b(1), b(2)})));
    REQUIRE(s.write(cc::span<byte const>({b(9), b(9)})).has_value());

    // rewind and read everything back
    REQUIRE(s.seek_to(0).value() == 0);
    cc::vector<byte> all = cc::vector<byte>::create_defaulted(4);
    REQUIRE(s.read(all).value() == 4);
    CHECK(bytes_equal(all, cc::span<byte const>({b(1), b(2), b(9), b(9)})));
}
