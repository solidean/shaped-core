...
// This measures a cumulative ladder of increasingly-complete operations on the SAME harness — median of 5, same machine and timer — so each row's delta isolates one component:
//
//   raw node alloc+free
//   + make_async_manual
...
// `metric` names the row as a guide metric; null records nothing.
// The intermediate rows are only interesting as deltas, so recording them would add gate noise without adding coverage.
...
void run(bool record)
{
    ...
    // The recorded rows: the allocator floor doubles as the contamination canary, since nothing here touches it — if it moves, the run is dirty.
    // The other three are the numbers the benchmark doc quotes.
