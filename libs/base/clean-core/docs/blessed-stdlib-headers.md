...
| `<variant>` `<tuple>` | for now   | Tier 3, and on the way out: `cc::variant` and `cc::tuple` now exist. Kept only until `sr::input_event` migrates off `std::variant` — that commit moves both headers to the deny list, here and in the root `.shaped-lint.yml`. Reach for the `cc::` types in anything new. |
