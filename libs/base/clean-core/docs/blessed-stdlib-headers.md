...
| `<chrono>`           | **no — via [`clean-core/common/time.hh`](../src/clean-core/common/time.hh)** | Seconds as a `double` are the vocabulary: `cc::current_time_steady_secs` for elapsed time, `cc::current_time_wall_secs` for a timestamp, and `cc::this_thread_sleep_secs` for a sleep. `<chrono>` is the most expensive header MSVC ships, so only `time.cc` and `thread.cc` include it. |
| `<limits>` `<thread>` | for now   | Tier 3: there is no `cc::` numeric limits or thread type yet. Nothing better exists to reach for today. |
| `<algorithm>`        | **no — via [`clean-core/algorithm/`](../src/clean-core/algorithm/sort.hh)** | `cc::sort` and its family cover the sorting, selection and partitioning half, and they do what `std::sort` structurally cannot: permute several parallel ranges under one order. The rest of `<algorithm>` — binary search above all — has no `cc::` answer yet, so a library that needs one re-allows the header scoped to the files that do. |
