...
The body resumes attributed to its caller's context instead of its own, so the span is wrong rather than missing.
Everything cc::async does with the context measures correct, and the loss happens inside the coroutine resume.
