...
public:
    ...
    /// Hand an operation over, inert.
    /// It never completes inline -- always from a later `wait`, and not even then until `arm` has run.
    void submit(io_operation* op);

    /// Say that the submitter has finished wiring `op` up, so it may now complete.
    ///
    /// The two-step exists because the submitter keeps touching the operation after handing it over -- registering it
...
private:
    struct entry
    {
        ...
        /// False between `submit` and `arm`, during which this entry is watched but never completed.
        bool armed = false;
