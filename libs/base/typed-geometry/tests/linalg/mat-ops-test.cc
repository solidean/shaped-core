#include "../approx.hh"

#include <nexus/test.hh>
#include <typed-geometry/linalg/mat.hh>
#include <typed-geometry/linalg/mat_ops.hh>
#include <typed-geometry/linalg/vec.hh>
#include <typed-geometry/scalar/angle.hh>

namespace
{
tg::mat3f sample3()
{
    return tg::mat3f::make_from_cols(tg::vec3f(1, 2, 3), tg::vec3f(0, 1, 4), tg::vec3f(5, 6, 0));
}

tg::mat4f sample4()
{
    return tg::mat4f::make_from_cols(tg::vec4f(1, 0, 2, 1), tg::vec4f(0, 3, 1, 0), tg::vec4f(2, 1, 0, 1),
                                     tg::vec4f(1, 1, 0, 2));
}
} // namespace

TEST("tg mat_ops - transpose")
{
    SECTION("rectangular transpose swaps the shape")
    {
        auto const m = tg::mat<3, 2, float>::make_from_cols(tg::vec2f(1, 2), tg::vec2f(3, 4), tg::vec2f(5, 6));
        auto const t = tg::transpose(m);

        for (int c = 0; c < 3; ++c)
            for (int r = 0; r < 2; ++r)
                CHECK((m[c, r]) == (t[r, c]));
    }

    SECTION("transpose is an involution")
    {
        auto const m = sample3();
        CHECK(tg::transpose(tg::transpose(m)) == m);
    }

    SECTION("the identity is symmetric")
    {
        CHECK(tg::transpose(tg::mat3f::identity) == tg::mat3f::identity);
    }
}

TEST("tg mat_ops - determinant")
{
    SECTION("identity has determinant one")
    {
        CHECK(tg::determinant(tg::mat2f::identity) == 1);
        CHECK(tg::determinant(tg::mat3f::identity) == 1);
        CHECK(tg::determinant(tg::mat4f::identity) == 1);
    }

    SECTION("2x2 by hand")
    {
        auto const m = tg::mat2f::make_from_cols(tg::vec2f(1, 3), tg::vec2f(2, 4));
        CHECK(tgtest::approx(tg::determinant(m), 1.0f * 4 - 2 * 3));
    }

    SECTION("3x3 by hand")
    {
        CHECK(tgtest::approx(tg::determinant(sample3()), 1.0f));
    }

    SECTION("4x4 by hand")
    {
        CHECK(tgtest::approx(tg::determinant(sample4()), -20.0f));
    }

    SECTION("the determinant is multiplicative")
    {
        auto const a3 = sample3();
        auto const b3 = tg::mat3f::make_from_cols(tg::vec3f(2, 1, 0), tg::vec3f(0, 3, 1), tg::vec3f(1, 0, 2));
        CHECK(tgtest::approx(tg::determinant(a3 * b3), tg::determinant(a3) * tg::determinant(b3)));

        auto const a4 = sample4();
        auto const b4 = tg::mat4f::make_from_cols(tg::vec4f(0, 1, 1, 2), tg::vec4f(3, 0, 1, 0), tg::vec4f(1, 2, 0, 1),
                                                  tg::vec4f(0, 1, 3, 1));
        CHECK(tgtest::approx(tg::determinant(a4 * b4), tg::determinant(a4) * tg::determinant(b4), 1e-3f));
    }

    SECTION("the determinant is invariant under transpose")
    {
        CHECK(tgtest::approx(tg::determinant(tg::transpose(sample3())), tg::determinant(sample3())));
        CHECK(tgtest::approx(tg::determinant(tg::transpose(sample4())), tg::determinant(sample4())));
    }

    SECTION("a rotation preserves volume")
    {
        auto const r = tg::mat3f::make_rotation_z(tg::angle_f::make_from_degree(37));
        CHECK(tgtest::approx(tg::determinant(r), 1.0f));
    }

    SECTION("a scaling multiplies the determinant by the factor per dimension")
    {
        CHECK(tgtest::approx(tg::determinant(tg::mat3f::identity * 2.0f), 8.0f));
    }

    SECTION("a singular matrix has determinant zero")
    {
        auto const m = tg::mat3f::make_from_cols(tg::vec3f(1, 2, 3), tg::vec3f(2, 4, 6), tg::vec3f(0, 1, 0));
        CHECK(tgtest::approx(tg::determinant(m), 0.0f));
    }
}

TEST("tg mat_ops - adjugate and cofactor")
{
    SECTION("adjugate(m) * m == determinant(m) * identity, even when singular")
    {
        auto const m = tg::mat3f::make_from_cols(tg::vec3f(1, 2, 3), tg::vec3f(2, 4, 6), tg::vec3f(0, 1, 0));
        CHECK(tgtest::approx(tg::adjugate(m) * m, tg::mat3f::identity * tg::determinant(m)));
    }

    SECTION("adjugate(m) * m == determinant(m) * identity, 2x2 and 4x4")
    {
        auto const m2 = tg::mat2f::make_from_cols(tg::vec2f(4, 1), tg::vec2f(7, 3));
        CHECK(tgtest::approx(tg::adjugate(m2) * m2, tg::mat2f::identity * tg::determinant(m2)));

        auto const m4 = sample4();
        CHECK(tgtest::approx(tg::adjugate(m4) * m4, tg::mat4f::identity * tg::determinant(m4)));
    }

    SECTION("a singular 4x4 still has an adjugate")
    {
        // the last column is the sum of the first two
        auto const m = tg::mat4f::make_from_cols(tg::vec4f(1, 0, 2, 1), tg::vec4f(0, 3, 1, 0), tg::vec4f(2, 1, 0, 1),
                                                 tg::vec4f(1, 3, 3, 1));
        CHECK(tgtest::approx(tg::determinant(m), 0.0f));
        CHECK(tgtest::approx(tg::adjugate(m) * m, tg::mat4f::zero));
    }

    SECTION("cofactor is the transposed adjugate")
    {
        auto const m = sample3();
        CHECK(tg::cofactor(m) == tg::transpose(tg::adjugate(m)));
    }

    SECTION("cofactor equals determinant times the inverse transpose")
    {
        auto const m = sample3();
        auto const expected = tg::transpose(tg::inverse(m)) * tg::determinant(m);
        CHECK(tgtest::approx(tg::cofactor(m), expected, 1e-3f));
    }
}

TEST("tg mat_ops - inverse")
{
    SECTION("3x3 round trip")
    {
        auto const m = sample3();
        CHECK(tgtest::approx(tg::inverse(m) * m, tg::mat3f::identity, 1e-4f));
        CHECK(tgtest::approx(m * tg::inverse(m), tg::mat3f::identity, 1e-4f));
    }

    SECTION("4x4 round trip")
    {
        auto const m = sample4();
        CHECK(tgtest::approx(tg::inverse(m) * m, tg::mat4f::identity, 1e-4f));
    }

    SECTION("2x2 round trip")
    {
        auto const m = tg::mat2f::make_from_cols(tg::vec2f(4, 1), tg::vec2f(7, 3));
        CHECK(tgtest::approx(tg::inverse(m) * m, tg::mat2f::identity, 1e-5f));
    }

    SECTION("a rotation inverts to its transpose")
    {
        auto const r = tg::mat3f::make_rotation_y(tg::angle_f::make_from_degree(63));
        CHECK(tgtest::approx(tg::inverse(r), tg::transpose(r), 1e-5f));
    }

    SECTION("a singular matrix inverts to zero")
    {
        auto const m = tg::mat3f::make_from_cols(tg::vec3f(1, 2, 3), tg::vec3f(2, 4, 6), tg::vec3f(0, 1, 0));
        CHECK(tg::inverse(m) == tg::mat3f::zero);

        auto const m4 = tg::mat4f::make_from_cols(tg::vec4f(1, 0, 2, 1), tg::vec4f(0, 3, 1, 0), tg::vec4f(2, 1, 0, 1),
                                                  tg::vec4f(1, 3, 3, 1));
        CHECK(tg::inverse(m4) == tg::mat4f::zero);
    }

    SECTION("an all-zero matrix inverts to zero")
    {
        CHECK(tg::inverse(tg::mat3f::zero) == tg::mat3f::zero);
    }
}
