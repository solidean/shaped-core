...
consteval class_list canonical_classes()
{
    ...
    {
        auto const c = tg::canonical(transform_flags(bits));
...
consteval bool verify_transform_flag_lattice()
{
    ...
    for (int a = 0; a <= int(transform_flags::all); ++a)
    {
        ...
        auto const ca = tg::canonical(fa);

        if (tg::canonical(ca) != ca)
            return false; // idempotent
        if (!tg::is_canonical(ca))
            return false;
        if (!tg::is_subclass(fa, ca))
            return false; // a flag set belongs to its own class
        if (tg::canonical(fa | ca) != ca)
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
        {
            auto const join = tg::canonical(classes[i] | classes[j]);

            if (tg::canonical(classes[j] | classes[i]) != join)
                return false; // commutative
            if (!tg::is_canonical(join))
                return false; // closed
            if (!tg::is_subclass(classes[i], join) || !tg::is_subclass(classes[j], join))
                return false; // an upper bound

            for (int k = 0; k < n; ++k)
            {
                // the LEAST upper bound: nothing containing both may fail to contain the join
                if (tg::is_subclass(classes[i], classes[k]) && tg::is_subclass(classes[j], classes[k])
                    && !tg::is_subclass(join, classes[k]))
                    return false;
            }
        }
...
static_assert(tg::is_canonical(tc::identity));
static_assert(tg::is_canonical(tc::translation));
static_assert(tg::is_canonical(tc::uniform_scaling));
static_assert(tg::is_canonical(tc::uniform_scaling_translation));
static_assert(tg::is_canonical(tc::scaling));
static_assert(tg::is_canonical(tc::scaling_translation));
static_assert(tg::is_canonical(tc::rotation));
static_assert(tg::is_canonical(tc::rigid));
static_assert(tg::is_canonical(tc::scaled_rotation));
static_assert(tg::is_canonical(tc::similarity));
static_assert(tg::is_canonical(tc::linear));
static_assert(tg::is_canonical(tc::affine));
static_assert(tg::is_canonical(tc::projective));
} // namespace
...
TEST("tg transform_flags - canonicalization")
{
    ...
    {
        CHECK(tg::canonical(transform_flags::uniform_scaling | transform_flags::non_uniform_scaling)
    ...
    SECTION("rotation with non-uniform scaling is a general linear map")
    {
        ...
        CHECK(tg::canonical(transform_flags::rotation | transform_flags::non_uniform_scaling) == tc::linear);
        CHECK(tg::has_all(tc::linear, transform_flags::general_linear));
    }

    SECTION("projection is the top of the lattice")
    {
        CHECK(tg::canonical(transform_flags::projection) == transform_flags::all);
...
TEST("tg transform_flags - is_subclass is not a bit-subset test")
{
    ...
    {
        CHECK(!tg::has_all(tc::affine, tc::similarity));
    }

    SECTION("the class test gets it right")
    {
        CHECK(tg::is_subclass(tc::similarity, tc::affine));
        CHECK(tg::is_subclass(tc::rigid, tc::similarity));
        CHECK(tg::is_subclass(tc::rotation, tc::rigid));
        CHECK(tg::is_subclass(tc::translation, tc::rigid));
        CHECK(tg::is_subclass(tc::affine, tc::projective));
        CHECK(tg::is_subclass(tc::uniform_scaling, tc::scaling_translation));
    }

    SECTION("containment is strict where it should be")
    {
        CHECK(!tg::is_subclass(tc::rigid, tc::rotation));
        CHECK(!tg::is_subclass(tc::rotation, tc::translation));
        CHECK(!tg::is_subclass(tc::scaling, tc::similarity));
        CHECK(!tg::is_subclass(tc::projective, tc::affine));
    }

    SECTION("every class contains itself")
    {
        CHECK(tg::is_subclass(tc::identity, tc::identity));
        CHECK(tg::is_subclass(tc::affine, tc::affine));
    }
}
...
TEST("tg transform_flags - bit operations")
{
    ...
    {
        CHECK(tg::has_any(tc::rigid));
        CHECK(!tg::has_any(tc::identity));
        CHECK(tg::has_all(tc::rigid, transform_flags::rotation));
        CHECK(tg::without(tc::rigid, transform_flags::rotation) == transform_flags::translation);
    }
}
