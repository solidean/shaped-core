...
That fix is to name a disabled test exactly, or to pass the bucket flag for a `manual` / `pgo_benchmark` test.
...
So `dev.py test hash-benchmark.cc` reports that the file's benchmarks are in the `pgo_benchmark` bucket, and `dev.py test hash-benchmark.cc --pgo-benchmarks` runs them.
