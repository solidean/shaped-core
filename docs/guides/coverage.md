...
`uv run dev.py coverage` collects **LLVM source-based coverage** for the test suite.
It builds an instrumented variant, runs the tests to produce raw counter data, merges it, and post-processes it into a machine-readable JSON report plus a short terminal summary.
This is the full reference; the loop itself is three subcommands — `run`, `merge`, `report`.
Back to [guides](_index.md).

Why LLVM source-based coverage: every default preset is Clang-based — `clang-cl` on Windows, `clang` on Linux, Homebrew `llvm` clang on macOS.
So one toolchain works everywhere (`-fprofile-instr-generate -fcoverage-mapping` → `llvm-profdata` → `llvm-cov`), and `llvm-cov export` emits JSON natively.
...
Coverage instrumentation is a build-time concern, gated by the `SC_COVERAGE` CMake option in the root [CMakeLists.txt](../../CMakeLists.txt).
When ON it adds the LLVM coverage flags to **every** target repo-wide; it is off for normal builds.
You don't set it by hand — the dedicated **`*-coverage` presets** turn it on.
They are RelWithDebInfo, matching the default tested preset so the suite is green.
They also configure into their own `build/<preset>/` dir, keeping instrumented artifacts out of your normal builds.
...
`coverage run` with no `--preset` uses the platform default above.
The pipeline per preset:
...
The **`.llvm-cov.json` suffix is deliberate**: it is the discoverable artifact a future `coverage_diag` MCP tool would read.
That mirrors how `build_diag` and `test_diag` read `build.json` / `test.json` today.
Build tooling on top of it rather than parsing terminal output.
...
`merge` is for combining coverage from several build configurations, or from accumulated runs — union the Windows and Linux numbers in CI before applying a gate, for instance.

## Tooling

`llvm-profdata` and `llvm-cov` must be present and **version-matched to the clang that built the binaries**; a mismatch silently corrupts the mapping.
dev.py resolves each one in that order: the `LLVM_PROFDATA` / `LLVM_COV` environment override, then `PATH`, then the directory of the configured compiler.
That last step is what finds them on Windows, where `clang-cl` ships them in the same LLVM `bin/`.
`uv run dev.py doctor` reports both with their versions.
[pgo.md](pgo.md) uses the same resolution for `llvm-profdata`.
