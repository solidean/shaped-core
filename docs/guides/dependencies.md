...
It refuses, with the manifest untouched, when either asset lacks a file `install_plan` needs.
v1.9.2607 is the case it caught: every path moved, and the Linux tarball dropped `WinAdapter.h`.
