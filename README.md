...
| base | [clean-net](libs/base/clean-net/) | `cnet` | Networking: sockets and datagrams, HTTP and WebSocket over a backend seam, name resolution, and a loopback dev server. The transport and the protocol clients are peers rather than layers, because a browser has no sockets and does have HTTP. Early stage. |
| data | [babel-data](libs/data/babel-data/) | `babel` | The externals-free base of babel: base64, JSON and markdown over clean-core alone. Link this when it is all you need. |
| data | [babel-serializer](libs/data/babel-serializer/) | `babel` | The rest of babel — serialization & deserialization of the formats that need more than clean-core. Each parses into an unopinionated native structure, with opinionated aggregators ("load an image", "load a mesh") on top. Wavefront OBJ, STL, glTF 2.0/GLB, SQLite, and PNG/JPEG/HDR/PFM images today. |
...
| graphics | [shaped-graphics](libs/graphics/shaped-graphics/) | `sg` | The graphics-API wrapper: a backend-agnostic `context`, `command_list`, and GPU resource types over per-backend static libraries (dx12/vulkan tier 1, metal/webgpu tier 2, opengl/webgl legacy; metal is real across the surface on Metal 4 (macOS / iOS 26, Apple silicon), presenting headless until shaped-rendering's window gains a cocoa arm; GPU timestamps are the remaining gap). Also home to the render-routine framework. |
