...
struct itrace::mca_instruction
{
    ...
    /// Timeline cycles for iteration 0, present only with -timeline.
    /// c_retired drives the "@N" suffix.
    ...
    /// Usage per resource, parallel to mca_result::resources.
    /// Empty when !valid.
...
/// The parsed llvm-mca analysis for one trace.
...
/// llvm-mca reports each drop as "<stdin>:<line>:<col>: error: ...", where line = fed position + 2 (line 1 is the .intel_syntax header).
/// Duplicate reports for one line collapse to one entry.
cc::vector<u32> parse_mca_dropped_lines(cc::string_view stderr_text);

/// Parse one CodeRegion of llvm-mca -json output into the model.
/// `surviving_trace_indices[i]` is the trace instruction index that mca's Instructions[i] corresponds to — the fed set minus the dropped positions, in order.
/// `trace_instruction_count` sizes the aligned output.
/// Pure, so a test can feed it a fixture.
/// On any structural mismatch the result degrades — available / per_instruction_valid reflect what could be trusted — rather than throwing.
