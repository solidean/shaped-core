...
/// Explicit and implicit operands both: the implicit stack traffic of push/pop/call/ret is included, and lands in the `frame` region once classified.
...
///
/// `owner_frame` receives the index into `frame_bases` of the owning frame.
/// It is -1 when the address sits above every tracked frame (an untracked caller), or is heap.
