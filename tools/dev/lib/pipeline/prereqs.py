...
def ensure_node_webgpu(root: Path, preset_name: str = "") -> None:
    ...
    if npm is None:
        ui.write_line("node-webgpu: npm not found — node runs of WebGPU tests will SKIP (install node, or use --runtime deno)")
        return
    ...
    with profile.span("node-webgpu", type="prereq"), ui.suspend():
        result = subprocess.run([npm, "ci", "--no-audit", "--no-fund"], cwd=js)
    if result.returncode != 0:
