...
/// Prose is one semantic point per line, never a reflowed block — so a sentence that ends in the *middle*
/// of a line is a finding.
...
/// The value is that the rule exists where the line is, so a human or an agent that did not know the
/// convention meets it at the moment it applies.
...
/// See its `.cc` for what that deliberately excludes — `e.g.`, list markers, inline code — and the
/// false-positive shapes that are known to survive.
