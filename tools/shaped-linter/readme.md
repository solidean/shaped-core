...
| `qualified-record-definition` | A header defines its types qualified — `struct cc::span { … };` — rather than opening the namespace around them. A forward declaration never fires, which is what leaves `fwd.hh` alone; `impl` and `custom` are exempt at any depth. The fix unwraps the namespace only when its body is a plain series of record definitions, so a function or an enum alongside costs the fix, not the finding. It rewrites to a name that has to be declared already, normally in that library's `fwd.hh` — a missing declaration surfaces as a compile error on the next build. |
